...
**/
  ...
  if(l_flg_tst_BCM_Status())
  { 
    l_flg_clr_BCM_Status();
...
void RainFunction(void)
{
  ...
  {
    l_u8_wr_RLS_RainIntensity(0);
  }
  else if(rbi_RainSensorError > 0)
  {
    l_u8_wr_RLS_RainIntensity(15);
  }
  else
  {
  #ifdef RAIN_INTENSITY
   /* Rain intensity VCO_423: Output value on LIN */
    l_u8_wr_RLS_RainIntensity(rb_RainInt_IntensityOutput_g);     
  #else
     l_u8_wr_RLS_RainIntensity(14);
  #endif
#else
  l_u8_wr_RLS_RainIntensity(14);
#endif /*_NO_RAIN_*/
...
void Humidity_Function(void)
{
  ...
  {
    l_u16_wr_RLS_DewPointTemp_Value(0X3FF);             //dew point temperature
    l_u8_wr_RLS_RelativeHumidity_value(0XFF);             //relative Humidity
    l_u8_wr_RLS_WinGlassTemp_Value(0XFF);               //window temperature   
  }
  else
  {        
    if (rw_DewPoint_g == cw_HUMTEMP_INIT_g)
    {
      l_u16_wr_RLS_DewPointTemp_Value(0X3fe);
    }    
    else
    {
      if (rw_DewPoint_g < cw_DEWPOINT_MAX_mg )                /* dew point should be less than or equal to Scheibe temperature */
      {                                                                      
        l_u16_wr_RLS_DewPointTemp_Value(rw_DewPoint_g);        
      }
      else
      {
        l_u16_wr_RLS_DewPointTemp_Value(cw_DEWPOINT_MAX_mg);
      }
    }
        
    if (rb_Humidity_g == cw_HUMTEMP_INIT_g)                   /* Relative Humidity output*/ /*VCO_061 name hase changed*/
    {
      l_u8_wr_RLS_RelativeHumidity_value(0XFE);
    }        
    else
    {        
      if(rb_Humidity_g <200)
      {
        l_u8_wr_RLS_RelativeHumidity_value(rb_Humidity_g);
      }
      else
      {
        l_u8_wr_RLS_RelativeHumidity_value(200);
      }
    }
   
    if(rw_Temperature_raw_g < 0x27b)        //-40°„C
    {
      rw_Temperature_Sensor_g = 0x27b;    
    }
    else if(rw_Temperature_raw_g > 0x3449)  //87.5°„C
    {
    ...
    {
      l_u8_wr_RLS_WinGlassTemp_Value(0xfe);
    }
    else
    {   
      if(((rw_Temperature_Sensor_g-635)/50) <= 253)   //-40~86°„C
      {       
        l_u8_wr_RLS_WinGlassTemp_Value(((rw_Temperature_Sensor_g-635)/50));
      }
      else
      {
        l_u8_wr_RLS_WinGlassTemp_Value(253);
      }
    }        
  }
}
...
void OutPutBrightnessValue(void)
{
  ...
  if((rbi_LightValuesInitialised_g == FALSE)|| (rbi_EEPROMLoad_g == TRUE))
  {
    ...
    {      
      l_u16_wr_RLS_FwBrightness(0X7fe);                                  //FW INITIAL
      l_u16_wr_RLS_AmbBrightness(0x7fe);
      l_u8_wr_RLS_IRBrightness(0xfe);      
      l_u8_wr_RLS_DriveSideSolar_Value(0xfe); /* Init/nicht verfuegbar */
      l_u8_wr_RLS_PassgSideSolar_Value(0xfe); /* Init/nicht verfuegbar */
    }
  }
  else
  {
    ...
    if(rb_RSControlFlags_g == cb_ONLY_HUMIDITY)
    {
      ...
      l_u16_wr_RLS_AmbBrightness(0x7FF);
      l_u8_wr_RLS_IRBrightness(0xFF);      
      l_u8_wr_RLS_DriveSideSolar_Value(0xFF); /* Init/nicht verfuegbar */
      l_u8_wr_RLS_PassgSideSolar_Value(0xFF); /* Init/nicht verfuegbar */
    }
    else if(l_bool_rd_RLS_LightSensorFault() == TRUE)                    /*  VDR469   set for error*/
    {
      l_u16_wr_RLS_AmbBrightness(0x7FF);      
      l_u16_wr_RLS_FwBrightness(0X7FF);
      l_u8_wr_RLS_IRBrightness(0xFF);
      l_u8_wr_RLS_DriveSideSolar_Value(0xFF); /* Init/nicht verfuegbar */
      l_u8_wr_RLS_PassgSideSolar_Value(0xFF); /* Init/nicht verfuegbar */
    }
    ...
    {     
      ...
      else
      {
        ...
        l_u16_wr_RLS_FwBrightness(rw_FW_brightness_l);

        if(get_rw_Amb_light16bit_g() < 0x7fd)                           /*VCO_031 correct variable 0x03FD*/
        {
          l_u16_wr_RLS_AmbBrightness(get_rw_Amb_light16bit_g());
        }
        else
        {
          l_u16_wr_RLS_AmbBrightness(0x7fd);
        }        
      }
      ...
      if ((rbi_SoSe_Enabled_g == TRUE)&& (TRUE == rbi_SolarValuesInitialised_g))
      {
        ...
        l_u8_wr_RLS_DriveSideSolar_Value(rw_SoSe_SunInt_2D_Le_l);
        ...
        l_u8_wr_RLS_PassgSideSolar_Value(rw_SoSe_SunInt_2D_Re_l);       
      }
      else
      {
        l_u8_wr_RLS_DriveSideSolar_Value(0xfe); /* Init/nicht verfuegbar */
...
void ApplLinInit(byte rb_LIN_State_l)
{
  ...
  byte rb_Index_l; 
  static const byte LinMsgDefault_RLSs_01[8] = {0,0,254,7,254,7,254,0};   /* id: 0x011 */
  static const byte LinMsgDefault_RLSs_02[8] = {254,251,251,251,251,3,0,0};   /* id: 0x023 */
