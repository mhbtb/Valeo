...
void GetLeWWS(void)
{
    ...
    if(l_flg_tst_BCM_01()){
        ...
        if((l_bool_rd_L_BCM_KL15s() == TRUE)||(rb_speed_in_kmh_g > cb_IGNORE_IGNITION_SPEED)) /*VCO_083*/
        {
          rbi_Ignition_g = TRUE;
...
//              || (l_u8_rd_L_BCM_WiperControl() == 3) // high speed
  ...
  {    // check if system is on -> to switch off
    ...
    if((rw_NoCommunicationCounter_g > CW_LIN_E2_IDLE_TIMOUT))
    {
      ...
      if ( rbi_ASICStandByModeRequest_g == FALSE)  /*VDR_526*/
      {
        if ( (rbi_Ignition_g== FALSE) &&(rbi_StandbyFunction_enabled_g == TRUE))
        {
...
void CopyRLSData(void)
{
  ...
  if(l_flg_tst_RS_01())                            // pruefen ob eigene Nachricht korrekt empfangen wurde
  {
    l_flg_clr_RS_01() ;                          // loeschen des Pruefflags
    rbi_ConnectedToMaster_g = 1;                   // set connected to Master   
  }
...
//  send_SplashRequirement();
  ...
  if ((l_u8_rd_L_RS_CollectiveError()==0) && (rbi_Ignition_g== FALSE) &&(rbi_StandbyFunction_enabled_g == TRUE))
  {
    rbi_ASICStandByModeRequest_g = 1;   // Set Request for Standby Mode
  }
  else
...
//  static const byte LinMsgDefault_FSs_01[8]  = {0x00,0x7C,0x00,0xFC,0x00,0x00,0xFF,0xFF}; /* id: 0x029 "FSs_01" */
  ...
  do
  {
    RS_01._c[rb_Index_l] = LinMsgDefault_RLSs_01[rb_Index_l];
...
static void check_for_errors(void)
{
  ...
  l_u8_wr_L_RS_RainSenHealth((bi_ROM_CHECK_FAILURE_g)||(bi_RAM_CHECK_FAILURE_g));
  ...
  l_u8_wr_L_RS_CollectiveError(l_u8_rd_L_RS_RainSenHealth()|| l_u8_rd_L_RS_RainStateError()|| l_u8_rd_L_RS_VoltError());
}
