...
static unsigned char rb_RLS_msgCounter_g=0;// for sending wiper request at least 3 times
/********************End***********************/
...
void GetLeWWS(void)
{
  ...
  static byte rb_TransmitionChanged_mg = 0;                                                                   /***********************Add the Auto_Close_Window Function in Rain Condition*************************/   
#ifdef CS_STANDBY  
  /*rb_Kodierbyte0_Update_g = rb_Kodierbyte0_g;  
  Standby_Switch = l_u8_rd_L_RLS_ModeEnable();                     //0=no change 1=off 2= on 
...
/****************************this is for Geely diagnose service********************************/    
  if((rbi_EEPROMBytesWritten_g == TRUE) && \
     (LINDATADUMP_READ_B2 != EEPROM_Struct_PD.EEPROMStructPD.EEKodierbyte2_g))
  {
    rab_EEPROM_Bytes_toWrite_g[0] = LINDATADUMP_READ_B2;
    rw_EEPROM_Address_to_Write_g = 268;                            //set adress to write
    rb_I2C_EEPROM_bytesToWrite_g = 1;                              //set number of bytes to 1
    rbi_EEPROMBytesToWrite_g = TRUE;
    rbi_EEPROMBytesWritten_g = FALSE; 
    rb_TransmitionChanged_mg = TRUE;
  }
  
  if( rb_TransmitionChanged_mg && (rbi_EEPROMBytesWritten_g == TRUE))
  {
    rab_EEPROM_Bytes_toWrite_g[0] = (rb_VariantID_g+1);
    rw_EEPROM_Address_to_Write_g = 343;                            //set adress to write
    rb_I2C_EEPROM_bytesToWrite_g = 1;                              //set number of bytes to 1
    rbi_EEPROMBytesToWrite_g = TRUE;
    rbi_EEPROMBytesWritten_g = FALSE; 
    rb_TransmitionChanged_mg = FALSE;
  }
/****************************this is for Geely diagnose service********************************/
  ...
  else if(l_u8_rd_L_All_Wdw_Closed()== 1)
  {
  ...
  { 
    ...
    rbi_RealParkContact_g = l_bool_rd_L_BCM_RS_WiperHPInfo();        // wiper position
  ...
  {
    rb_LSPotiStage_g = (cab_PotistageMapping_mg[rb_LightSensorCoding_g]+rb_Empfindlichkeit_LS_l)-3;    //Updated in baseline 0008 2018.4.17
...
{  
  ...
  if(    ((l_u8_rd_L_BCM_RS_RainSwSts() != 5)    //auto wiper
...
void RainFunction(void)
{
  ...
  else
  {
    if(l_u8_rd_L_BCM_RS_RainSwSts() == 5)
    {
      if(rb_WiperSpeed_g!=0)
      {
        ...
        {
          newWiperSpeedLin = wiperSpeed7to2(l_u8_rd_L_RS_WiperAutoSpReq(), rb_WiperSpeed_g, rb_wiperSpeedHysteresis);  //continuous mode
        }  
      }
      else
      {
  ...
  if((rb_WiperSpeed_g > 0) && (rb_WiperSpeedAlt_g == 0))
  {
    rb_RLS_msgCounter_g = 3;
  }
  
  if((rb_RLS_msgCounter_g != 0) && (rb_WiperSpeed_g == 0))
  {
    l_u8_wr_L_RS_WiperAutoSpReq(1);
  }
...
void LightFunction(void)
{
 ...
 /* PRQA S 3415, 4112 1 */ /* << Getter functions must not have side effects >>*/
  if ((((((IS_LS_ANY_ERRORCOUNTER()) > 0))||((l_u8_rd_L_Status_Light_Sens()) > 0))||(((rbi_UnderVoltage_g) == TRUE))
  ...
  if(l_u8_rd_L_Status_Light_Sens() == TRUE)                                                                               //sensor in error
  {
  ...
  { /* PRQA S 3415 1 */ /* << Swapping expressions not feasible for performance >>*/
    ...
    {
      rb_TempLightOn_l = l_u8_rd_L_Light_Switch_Request();                                                               /* stay in old light mode and do */
    }
  }
      ...
      && ((rb_RSControlFlags_g == cb_HUMIDITY_SOLAR )||(rb_RSControlFlags_g == cb_ONLY_HUMIDITY)))
  {
    ...
    l_u8_wr_L_Light_Switch_Request(TRUE);   
  } 
  else
  {
    /* PRQA S 434, 436, 781 7 */ /* <<usind Micro Controller specific command > */
    DISABLE_SCIINTERRUPT();
    l_u8_wr_L_Light_Switch_Request(rb_TempLightOn_l);                                                                    // Turn Light on = TRUE
    ENABLE_SCIINTERRUPT();
  }  
  ...
  l_u8_wr_L_Light_Switch_Reason(rb_ReasonForLightOn_l);     
}
...
void Humidity_Function(void)
{
  ...
  {
    l_u16_wr_L_DP_Temp(1396);                                //dew point temperature
    l_u8_wr_L_RH_Sens(202);      //Blake_Check 2018_03_10                             //relative Humidity
    l_u16_wr_L_Window_Temp(1396);                            //window temperature    
  }
  else
  {
    ...
    else if(rw_Temperature_raw_g > 0x3953)                   // The temperature greater than 60,4 the tool doesnot support
    {
      rw_Temperature_Scheibe_g = 0x3953;
    ...
    {
      if(((rw_Temperature_Scheibe_g -725)/10) < 1395)
      {        
        l_u16_wr_L_Window_Temp(((rw_Temperature_Scheibe_g -725)/10));
      }
      else
      {         
        l_u16_wr_L_Window_Temp(1395);
      }
    } 
    ...
    else
    {
      if (rw_DewPoint_g < cw_DEWPOINT_MAX_mg )                /* dew point should be less than or equal to Scheibe temperature */
      {
        ...
        l_u16_wr_L_DP_Temp(rw_DewPoint_g); 
        
      }
      else
      {
        l_u16_wr_L_DP_Temp(cw_DEWPOINT_MAX_mg);
      }
    }
    ...
    {        
      l_u8_wr_L_RH_Sens(rb_Humidity_g);
    }
...
//      {
//        
//        l_u16_wr_L_Window_Temp(((rw_Temperature_Sensor_g-685)/50));
//      }
//      else
//      {
//        l_u16_wr_L_Window_Temp(1);
//      }
//    }        
  }
}
...
//  word rw_Hellighkeitsthrs;                                              /*VCO_103 local variable for brightness boost*/
  ...
  unsigned long rl_HUD_Brightness;
  unsigned long rl_HUD_AMB_Brightness;
  unsigned long rl_HUD_FW_Brightness;
  ...
  if((rbi_LightValuesInitialised_g == FALSE)|| (rbi_EEPROMLoad_g == TRUE))
  {
    ...
    {      
      l_u8_wr_L_Light_HudBrightnessValue(0X7E);                          // FW Init
      l_u8_wr_L_Light_HudBrightnessUnit(2);
      l_u8_wr_L_Value_Solar_Right_Sens(0xFE);         //Blake_Check 2018_03_10                   // Init 
      l_u8_wr_L_Value_Solar_Left_Sens(0xFE);          //Blake_Check 2018_03_10                   // Init
    }
  }
  else
  {
    ...
    {
      /*Incase of only Feuchte and solar frame preset then set the light ON, FW and IR to init value for safety reason*/ 
      l_u8_wr_L_Light_HudBrightnessValue(0X7F);                          //FW Init
      l_u8_wr_L_Light_HudBrightnessUnit(2);
      l_u8_wr_L_Value_Solar_Right_Sens(0xFF);       //Blake_Check Even never run here,but make up is better.                      /* Init*/
      l_u8_wr_L_Value_Solar_Left_Sens(0xFF);                             /* Init*/
      //l_u8_wr_LS_Helligkeit_IR(0xFE);      
    }
    else if(l_u8_rd_L_Err_State_Light_Sens() == TRUE)                    /*  VDR469   set for error*/
    {
      //l_u8_wr_LS_Helligkeit_IR(0xFF);                                  // IR Helligkeit Fehler
      l_u8_wr_L_Light_HudBrightnessValue(0X7F);
      l_u8_wr_L_Light_HudBrightnessUnit(2);
      l_u8_wr_L_Value_Solar_Right_Sens(0xFF);                            /* Init*/
      l_u8_wr_L_Value_Solar_Left_Sens(0xFF);                             /* Init*/
    }
    else
    {
//Blake Marked it 2018_03_10  
//      if(rb_RSControlFlags_g == cb_HUMIDITY_SOLAR )
//      {
//        l_u8_wr_L_Light_HudBrightnessValue(0X7F);                        // FW Helligkeit Fehler
//        l_u8_wr_L_Light_HudBrightnessUnit(2);
//      }
//      else
//      {
...
//      //l_u4_wr_LS_Helligkeit_FW_boost(rb_Helligkeitsboost_l);
      ...
      rl_HUD_AMB_Brightness = 15*((unsigned long)(get_rw_Amb_light16bit_g()));
      rl_HUD_FW_Brightness = (unsigned long)((3*((unsigned long)(get_rl_HUD_light32bit_g())))>>1);
      ...
      GetFiveTableValue_50ms(rl_HUD_AMB_Brightness,rl_HUD_FW_Brightness);   
      
      /*************5000ms for average output**************/
      rl_HUD_Brightness  = CalcHUDMean_1000ms();
     
      /***********Output to LIN Interface**************/
      if(rl_HUD_Brightness < 100)
      {
        l_u8_wr_L_Light_HudBrightnessValue(rl_HUD_Brightness);                        // FW Helligkeit Fehler
        l_u8_wr_L_Light_HudBrightnessUnit(0);          
      }else if((rl_HUD_Brightness + 50)/100 < 127)
      {
        l_u8_wr_L_Light_HudBrightnessValue((rl_HUD_Brightness + 50)/100);             // FW Helligkeit Fehler
        l_u8_wr_L_Light_HudBrightnessUnit(1);       
      }else if((rl_HUD_Brightness + 500)/1000 < 127)
      {
        l_u8_wr_L_Light_HudBrightnessValue((rl_HUD_Brightness + 500)/1000);            // FW Helligkeit Fehler
        l_u8_wr_L_Light_HudBrightnessUnit(2);      
      }else
      {
        l_u8_wr_L_Light_HudBrightnessValue(126);                        // FW Helligkeit Fehler
        l_u8_wr_L_Light_HudBrightnessUnit(2);  
      }
      ...
      if ((rbi_SoSe_Enabled_g == TRUE) && (TRUE == rbi_SolarValuesInitialised_g))
      {
        ...
        rw_SoSe_SunInt_2D_Le_l=((rw_SoSe_SunInt_2D_Le_l>255)?255:rw_SoSe_SunInt_2D_Le_l);
        l_u8_wr_L_Value_Solar_Left_Sens(rw_SoSe_SunInt_2D_Le_l);
        
        rw_SoSe_SunInt_2D_Re_l=(rw_SoSe_SunInt_2D_Re_l>>3)*5;
        rw_SoSe_SunInt_2D_Re_l=((rw_SoSe_SunInt_2D_Re_l>255)?255:rw_SoSe_SunInt_2D_Re_l);
        l_u8_wr_L_Value_Solar_Right_Sens(rw_SoSe_SunInt_2D_Re_l);       
      }
      else
      {
        l_u8_wr_L_Value_Solar_Right_Sens(0xFF); /* Init/nicht verfuegbar */
        l_u8_wr_L_Value_Solar_Right_Sens(0xFF); /* Init/nicht verfuegbar */
      }
#endif
...
static void CheckErrors(void)
{
  l_u8_wr_L_Err_State_Cal_RS_Reserved(1);                                                 //custorm defined, but we do not support
  ...
  l_bool_wr_L_RS_RainSensorError((rb_RS_LS_Error1_g != 0)||(rb_RS_LS_Error2_g != 0)||(rb_RainS_Error1_g != 0) ||(rbi_HammingError_g != 0));  
  l_u8_wr_L_Err_State_Light_Sens((rb_RS_LS_Error1_g != 0)||(rb_RS_LS_Error2_g != 0)||(rb_LightS_Error1_g != 0)||(rbi_HammingError_g != 0)||(rbi_CRC_ChangedByDiagnostic_g));
  l_u8_wr_L_Status_Light_Sens((rb_RS_LS_Error1_g != 0)||(rb_RS_LS_Error2_g != 0)||(rb_LightS_Error1_g != 0)||(rbi_HammingError_g != 0)||(rbi_CRC_ChangedByDiagnostic_g));  
  l_bool_wr_L_RS_SolarSensorError((rb_RS_LS_Error1_g != 0)||(rb_RS_LS_Error2_g != 0)||(rb_LightS_Error1_g != 0)||(rbi_HammingError_g != 0)||(rbi_CRC_ChangedByDiagnostic_g));
}
...
void ApplLinInit(byte rb_LIN_State_l)
{
  ...
  byte rb_Index_l; 
  static const byte LinMsgDefault_RLSs_01[8] =  {252,0,0,48,79,13,255,255};   /* id: 0x015 "RLSs_01" */
  static const byte LinMsgDefault_RLSs_02[8] = {255,255,0,240,0,0,0,0}; /* id: 0x016 "RLSs_02" */
...
unsigned long CalcHUDMean_1000ms(void)
{
  ...
  return ((HUDEverageValue[0] + HUDEverageValue[1])/2);// + HUDEverageValue[2] + HUDEverageValue[3] + HUDEverageValue[4]) / 5);
}
...
void GetFiveTableValue_50ms(unsigned long Fw,unsigned long Amb)
{
  ...
  if(rb_k_cycleIn50ms >= 5)
  {
    ...
    {
      FW_Average_In_50ms =(FW_Average_In_50ms+  rb_storeFW_ValueEvery50ms[i])/2;
    }
    
    AMB_Average_In_50ms=rb_storeAMB_ValueEvery50ms[0];
    for(unsigned char i = 1;i < 5;i++)
    {
      AMB_Average_In_50ms =(AMB_Average_In_50ms+  rb_storeAMB_ValueEvery50ms[i])/2;
    } 
    
    if((FW_Average_In_50ms_Old>0)&&(FW_Average_In_50ms>0)&&(AMB_Average_In_50ms_Old>0)&&(AMB_Average_In_50ms>0))
    {
      if(((FW_Average_In_50ms_Old > FW_Average_In_50ms)&&(AMB_Average_In_50ms_Old > AMB_Average_In_50ms)))
      {
        FW_Slope = (double)FW_Average_In_50ms / (double)FW_Average_In_50ms_Old;
        AMB_Slope = (double)AMB_Average_In_50ms / (double)AMB_Average_In_50ms_Old;
        if(FW_Slope > AMB_Slope)
        {
           AMB_Average_In_50ms =(unsigned long)(FW_Slope * AMB_Average_In_50ms_Old);
        }
        else
        {
           FW_Average_In_50ms = (unsigned long)(AMB_Slope * FW_Average_In_50ms_Old);
        }
      }
      else if((FW_Average_In_50ms_Old < FW_Average_In_50ms)&&(AMB_Average_In_50ms_Old < AMB_Average_In_50ms))
      {
        FW_Slope = (double)FW_Average_In_50ms / (double)FW_Average_In_50ms_Old;
        AMB_Slope = (double)AMB_Average_In_50ms / (double)AMB_Average_In_50ms_Old;
        if(FW_Slope > AMB_Slope)
      ...
      {
        FW_Average_In_50ms = (unsigned long)( (FW_Average_In_50ms_Old+FW_Average_In_50ms)/2 );
        AMB_Average_In_50ms = (unsigned long)( (AMB_Average_In_50ms_Old +AMB_Average_In_50ms)/2);     
      }
    }
