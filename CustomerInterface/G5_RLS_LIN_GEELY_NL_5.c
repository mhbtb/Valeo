...
void GetLeWWS(void)
{
  ...
  static byte rb_TransmissionValueValeo_l;  /****Add by Blake_LI 2018_08_20 for RLS_VDR_1068*****/
  static byte rb_TransmissionValueCustomer_l;  /****Add by Blake_LI 2018_08_20 for RLS_VDR_1068*****/
  static byte rb_VariantIDValeo_l;/****Add by Blake_LI 2018_08_20 for RLS_VDR_1068*****/
  static byte rb_TransmitionChanged_mg = 0;   
...
/****************************this is for Geely diagnose service********************************/    
  rb_TransmissionValueValeo_l = rb_Kodierbyte2_g; 
  rb_TransmissionValueCustomer_l = LINDATADUMP_READ_B2;
  
  if((rb_TransmissionValueValeo_l != rb_TransmissionValueCustomer_l) && ((rbi_EEPROMBytesToWrite_g == FALSE)||(rbi_EEPROMBytesWritten_g == TRUE)))
  {
    rab_EEPROM_Bytes_toWrite_g[0] = rb_TransmissionValueCustomer_l;
  ...
  rb_VariantIDValeo_l = rb_VariantID_g;
  if((rb_TransmitionChanged_mg == TRUE)&&((rbi_EEPROMBytesToWrite_g == FALSE)||(rbi_EEPROMBytesWritten_g == TRUE)))
  {
    rab_EEPROM_Bytes_toWrite_g[0] = (rb_VariantIDValeo_l+1);
...
/****************************this is for Geely diagnose service********************************/
  ...
  else if((l_u8_rd_L_All_Wdw_Closed()== 1) ||(l_u8_rd_L_All_Wdw_Closed()== 3))//xsy VCO_512 20181114 customer need Invalid is the same Complete Close
  {
  ...
  {                                                                     // check if system is on -> to switch off 
    ...
    {     
      ...
      else 
      {
        ...
        if ((l_u8_rd_L_All_Wdw_Closed()== 1)||(l_u8_rd_L_All_Wdw_Closed()== 3)||(rw_CommandCloseWindowTimout_mg > 6000))//xsy VCO_512 20181114 customer need Invalid is the same Complete Close
        {
...
void Humidity_Function(void)
{
  ...
  {
                                                             /*the temperature of screen ,the range is -39.6~60.4*/
    if(rw_Temperature_raw_g < 0x2D5)                         // The temperature less than -39,5 the tool doesnot support 
    {
      rw_Temperature_Scheibe_g = 0x2D5;
    }
    else if(rw_Temperature_raw_g > 0x3953)                   // 99.9 is the largest value for GEELY.The temperature greater than 60,4 the tool doesnot support
    {
...
void VoltageDetect(void)
{
  ...
  /***2018_08_20   Blake_LI updated the voltage based on the requirment***/
  if(rbi_OverVoltage_g == TRUE)    /***If voltage is over 17.4V,this variable will be set to TRUE***/     
  {
    l_u8_wr_L_Err_State_Over_Volt(1);  //voltage error statue
    
  }
  else                                  /***If voltage is under 17.4V,this variable will be clear***/ 
  {
...
//      //l_u4_wr_LS_Helligkeit_FW_boost(rb_Helligkeitsboost_l);
      ...
      {
        l_u8_wr_L_Value_Solar_Left_Sens(0xFE); /* Init/nicht verfuegbar */
        l_u8_wr_L_Value_Solar_Right_Sens(0xFE); /* Init/nicht verfuegbar */
      }
#endif
...
static void CheckErrors(void)
{
  ...
  /*******************Blake_Li 2018_08_20   If over_voltage then set the signal "L_Status_Light_Sens",else clear.***********/
  l_u8_wr_L_Status_Light_Sens((rbi_OverVoltage_g != 0)||(rb_RS_LS_Error1_g != 0)||(rb_RS_LS_Error2_g != 0)||(rb_LightS_Error1_g != 0)||(rbi_HammingError_g != 0)||(rbi_CRC_ChangedByDiagnostic_g));  
  /***************************************************************End*******************************************************/
...
void ApplLinInit(byte rb_LIN_State_l)
{
  ...
  byte rb_Index_l; 
  static const byte LinMsgDefault_RLSs_01[8] =  {12,0,0,48,79,13,255,255};   /*old is 252 id: 0x015 "RLSs_01" */
