...
void RainFunction(void)
{
  ...
  else  
  {
    if(IsAutoWiper())
    {
      if(rb_WiperSpeed_g!=0)
      {
         if(l_u8_rd_S_WipeRequst() > 0) 
         {
           rb_CurrentWipeSpdReqst_l= (((l_u8_rd_S_WipeRequst()) ^ 0x03));
         }
      ...
      if(newWiperSpeedLin > 0)
      {
        l_u8_wr_S_WipeRequst((newWiperSpeedLin ^ 0x03));   
      }
      else
      {
        l_u8_wr_S_WipeRequst(0);
      }
    }
    else
    {
...
void ApplLinInit(byte rb_LIN_State_l)
{
  ...
  byte rb_Index_l; 
  static const byte LinMsgDefault_RLSs_01[2] = {0x00,0x00};
  //static const byte LinMsgDefault_RLSs_02[2] = {0x00,0x00};
  
  rb_Index_l = 0;
  do
  {
    AUTOWIPE._c[rb_Index_l] = LinMsgDefault_RLSs_01[rb_Index_l];
    //MOTOR._c[rb_Index_l] = LinMsgDefault_RLSs_02[rb_Index_l];
    rb_Index_l++;
  } while (rb_Index_l < 2);
