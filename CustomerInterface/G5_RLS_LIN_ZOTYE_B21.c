...
void GetLeWWS(void)
{
  ...
  if (l_bool_rd_BDM_LIN1_WindowOpen()== 1)                                /*Window Open*/
  {
  ...
  else if(l_bool_rd_BDM_LIN1_WindowOpen()== 0)
  {
  ...
  if(l_flg_tst_BDM_LIN1_1())
  { 
    l_flg_clr_BDM_LIN1_1();
    rw_NoCommunicationCounter_g = 0;                                 // reset no communication counter
    rbi_ConnectedToMaster_g = 1;   
    rbi_RealParkContact_g = l_bool_rd_BDM_LIN1_WM_WiperHomePosition();            // wiper position
    rbi_LightSensitivityRecieved_g = TRUE;
    
    rb_speed_in_kmh_g  = ((l_u16_rd_BDM_LIN1_VehicleSpeed())+((l_u16_rd_BDM_LIN1_VehicleSpeed())>>3))/20;// N*0.05625      
    if((l_u16_rd_BDM_LIN1_VehicleSpeed()) > 4533)
    {
    ...
    rb_KL15_mg=((l_u8_rd_BDM_LIN1_PowerMode() ==2)||(l_u8_rd_BDM_LIN1_PowerMode() ==3) || (l_u8_rd_BDM_LIN1_PowerMode() ==4)|| (l_u8_rd_BDM_LIN1_PowerMode() ==5));
    ...
    {                                                                    /* ignore ignition off if speed higher 4km/h*/
      ...
      {
        if((l_u8_rd_BDM_LIN1_SCCM() == 2)                         //automatic
      ...
      if(l_u8_rd_BDM_LIN1_SCCM() == 2)                            // copy automatic wiping mode
      {
    ...
    rb_PotiStageFromBox_g = ((l_u8_rd_BDM_LIN1_RS_Sensitivity() > 4)?4:(l_u8_rd_BDM_LIN1_RS_Sensitivity()));
    
    if (l_bool_rd_BDM_LIN1_WashStatus())                                 /* Washpump activ? 0x5:Front washing*/
    {
  ...
  {                                                                     // check if system is on -> to switch off 
    ...
    {     
      ...
      else 
      {
        ...
        if ((l_bool_rd_BDM_LIN1_WindowOpen()== 0)||(rw_CommandCloseWindowTimout_mg > 6000))
        {
...
{  
  ...
  if(    ((l_u8_rd_BDM_LIN1_SCCM() != 2)    //auto wiper
...
void RainFunction(void)
{
  ...
  else  
  {
    if(l_u8_rd_BDM_LIN1_SCCM() == 2)
    {
      if(rb_WiperSpeed_g!=0)
      {
         ...
         else if (wiperSpeed7to2(l_u8_rd_RLS_RS_WiperSpeed(), rb_WiperSpeed_g, rb_wiperSpeedHysteresis) != 0)
         {
           newWiperSpeedLin = wiperSpeed7to2(l_u8_rd_RLS_RS_WiperSpeed(), rb_WiperSpeed_g, rb_wiperSpeedHysteresis);  //continuous mode
         }   
      }
      else
      {
    ...
    l_u8_wr_RLS_RS_WiperSpeed(newWiperSpeedLin);                     // Wischergeschwindigkeit uebergeben
  ...
  {
    l_u8_wr_RLS_RS_RainLevel(0);
  }
  else if(rbi_RainSensorError > 0)
  {
    l_u8_wr_RLS_RS_RainLevel(15);
  }
  else
  {
  #ifdef RAIN_INTENSITY
   /* Rain intensity VCO_423: Output value on LIN */
    l_u8_wr_RLS_RS_RainLevel(rb_RainInt_IntensityOutput_g);     
  #else
     l_u8_wr_RLS_RS_RainLevel(14);
  #endif
#else
  l_u8_wr_RLS_RS_RainLevel(14);
#endif /*_NO_RAIN_*/
  ...
  l_bool_wr_RLS_RS_Splash(rbi_RSWipeFastDmd);                                                                      //splash mode
...
void LightFunction(void)
{
 ...
 /* PRQA S 3415, 4112 1 */ /* << Getter functions must not have side effects >>*/
  if ((((((IS_LS_ANY_ERRORCOUNTER()) > 0))||((l_bool_rd_RLS_LS_Error()) > 0))||(((rbi_UnderVoltage_g) == TRUE))
  ...
  if(l_bool_rd_RLS_LS_Error() == TRUE)                                                                                 //sensor in error
  {
  ...
  { /* PRQA S 3415 1 */ /* << Swapping expressions not feasible for performance >>*/
    ...
    {
      rb_TempLightOn_l = l_bool_rd_RLS_LS_HeadLamp();                                                                      /* stay in old light mode and do */
    }
  } 
...
void Humidity_Function(void)
{
  ...
  {
    l_u16_wr_RLS_DewPoint(0X3FF);              //dew point temperature
    l_u8_wr_RLS_Humidity(0XFF);             //relative Humidity
    l_u16_wr_RLS_WindshieldTemp(0X3FF);      //window temperature   
  }
  else
  {
    ...
    else
    {
      ...
      {
        l_u16_wr_RLS_DewPoint(1000);
      }
    }
        
    if (rb_Humidity_g == cw_HUMTEMP_INIT_g)                   /* Relative Humidity output*/ /*VCO_061 name hase changed*/
    {
      l_u8_wr_RLS_Humidity(0);
    }        
    else
    { 
      if(rb_Humidity_raw_g< 200)
      {
        l_u8_wr_RLS_Humidity(rb_Humidity_raw_g);
      }
      else
      {
        l_u8_wr_RLS_Humidity(200);
      }
    }
    ...
    else if(rw_Temperature_raw_g > 0x29bd)/* The temperature greater than 60 the tool doesnot support */
    {
      rw_Temperature_Sensor_g = 0x29bd;
    }
    else
    {
    ...
    {
      l_u16_wr_RLS_WindshieldTemp(0);
    }
    else
    {   
      if(((rw_Temperature_Sensor_g-685)/10) < 1000)
      {        
        l_u16_wr_RLS_WindshieldTemp(((rw_Temperature_Sensor_g-685)/10));
      }
      else
      {
        l_u16_wr_RLS_WindshieldTemp(1000);
      }
    }        
  }
}
...
void StandByMode(void)
{
  ...
  {
    l_u8_wr_RLS_RS_CloseRequest(1);                  // send close window command
...
void OutPutBrightnessValue(void)
{
  ...
  if((rbi_LightValuesInitialised_g == FALSE)|| (rbi_EEPROMLoad_g == TRUE))
  {
    ...
    {      
      l_u16_wr_RLS_LS_Forward(0x3FE);                                   //FW invailed
      l_u8_wr_RLS_LS_Global(0xFE);
      l_u8_wr_RLS_SolarRight(0xFF);                     //SOLAR invailed
      l_u8_wr_RLS_SolarLeft(0xFF);                     //SOLAR invailed
    }
  }
  else
  {
    ...
    if(rb_RSControlFlags_g == cb_ONLY_HUMIDITY)
    {
      ...
      l_u16_wr_RLS_LS_Forward(0X3FE);                                  //FW invailed
      l_u8_wr_RLS_LS_Global(0xFE);  
      l_u8_wr_RLS_SolarRight(0xFE);                     //SOLAR invailed
      l_u8_wr_RLS_SolarLeft(0xFE);                     //SOLAR invailed
    }
    else if(l_bool_rd_RLS_LS_Error() == TRUE)                    /*  VDR469   set for error*/
    {
      l_u16_wr_RLS_LS_Forward(0X3FF);
      l_u8_wr_RLS_LS_Global(0xFF);                                  // IR Helligkeit Fehler
      l_u8_wr_RLS_SolarRight(0xFF);                            /* Init*/
      l_u8_wr_RLS_SolarLeft(0xFF);                             /* Init*/
    }
    else
    {
      ...
      {
         l_u16_wr_RLS_LS_Forward(0X3FF);
         l_u8_wr_RLS_LS_Global(0xFF); 
      }
      else
      {
         if(get_rw_FW_light16bit_g() < 0x03FD) /*VCO_031 correct variable*/
        {
          l_u16_wr_RLS_LS_Forward(get_rw_FW_light16bit_g());
        }
        else
        {
          l_u16_wr_RLS_LS_Forward(0x03FD);   // FW Helligkeit max
      ...
      {
        l_u8_wr_RLS_LS_Global((byte)rb_IrLightDiv8_g);
      }
      else
      {
        l_u8_wr_RLS_LS_Global(0xFD);     // IR Helligkeit max
      }
...
static void CheckErrors(void)
{
  ...
  l_bool_wr_RLS_RS_Error((rb_RS_LS_Error1_g != 0)||(rb_RS_LS_Error2_g != 0)||(rb_RainS_Error1_g != 0) ||(rbi_HammingError_g != 0));  
  l_bool_wr_RLS_LS_Error((rb_RS_LS_Error1_g != 0)||(rb_RS_LS_Error2_g != 0)||(rb_LightS_Error1_g != 0)||(rbi_HammingError_g != 0)||(rbi_CRC_ChangedByDiagnostic_g));
 }
...
void ApplLinInit(byte rb_LIN_State_l)
{
  ...
  byte rb_Index_l; 
  static const byte LinMsgDefault_RLSs_02[8] = {0,0,0,0,0,0,0,0};          /* 0x11"RLS_2" */
  static const byte LinMsgDefault_RLSs_01[8] = {0,0,254,128,255,14,0,0};    /* 0x10"RLS_1" */
  
  rb_Index_l = 0;
  do
  {
    RLS_1._c[rb_Index_l] = LinMsgDefault_RLSs_01[rb_Index_l];
    RLS_2._c[rb_Index_l] = LinMsgDefault_RLSs_02[rb_Index_l];
    rb_Index_l++;
  } while (rb_Index_l < 8);
