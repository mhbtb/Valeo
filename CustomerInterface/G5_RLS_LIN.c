...
void GetLeWWS(void) /*Information from SCS*/
{
  ...
  if(l_flg_tst_BCM1_LIN1_01())               /*Master Request?*/
  {
    l_flg_clr_BCM1_LIN1_01();                 /*loesche flag*/
    ...
    if((l_bool_rd_Klemme_15() == TRUE)||(rb_speed_in_kmh_g > cb_IGNORE_IGNITION_SPEED)) /*VCO_083*/
    {
      ...
      {
        if(((l_bool_rd_WH_Intervall() != FALSE)
      ...
      if(l_bool_rd_WH_Intervall()== TRUE) // copy automatic wiping mode
      {
    ...
    if(l_u8_rd_ESP_v_Signal_8Bit() < 100)/*Check for max value   speed faster than 200 (255km/h) */
    {
      rb_speed_in_kmh_g = (byte)(((word)(l_u8_rd_ESP_v_Signal_8Bit())*41)>>4); /*recalibrate Data multily by 2,56*/
    }
    else
    {
      if(l_u8_rd_ESP_v_Signal_8Bit() == 0xFF)
      {
        rb_speed_in_kmh_g = 75;                           /* set speed to (75 km/h)*/
      }
      else 
      {
        if(l_u8_rd_ESP_v_Signal_8Bit() < 0xFE)
        {
...
void GetLsWWS(void)
{
  ...
  {
    if(((rb_Wischer_Parklage_l == FALSE)) && (rw_StartupWipeCommandTimOut_mg < cw_CLEARWIPE_DELAY_s)) 
    {
...
void CopyRLSData(void)
{
         ...
         &&(rb_WiperSpeed_g == 0))
  {
    ...
    l_u8_wr_RS_Wischergeschwindigkeit(1);
  }
  ...
  if ((l_bool_rd_RS_Error()==FALSE) && ((rbi_WindowOpenIgnitionWasOn_g == TRUE) && (rbi_Ignition_g== FALSE)) &&((rb_Verglasun_offenDebounce_mg > cb_VERGLSUNG_OPEN_DELAY_mg)&& (rbi_StandbyFunction_enabled_g == TRUE)))
  {
    rbi_ASICStandByModeRequest_g = 1;   // Set Request for Standby Mode
  }
  else
  ...
  else
  {
   ...
   l_u8_wr_RS_Regenmenge( rb_RainInt_IntensityOutput_g);
#else
   l_u8_wr_RS_Regenmenge( 14);
#endif
  }
  /*VDR_859 Interval wipe in manual mode for no rain variant*/
...
void ApplLinInit(byte rb_LIN_State_l)
{
  ...
  static const byte LinMsgDefault_RLSs_01[8] = {0x00,0x10,0xF0,0xFE,0xFE,0xEF,0xFF,0xFF}; /* id: 0x023 "RLSs_01" */
  static const byte LinMsgDefault_SoSes_01[8]= {0xFE,0xFE,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF};/* id: 0x30"SoSes_01" */
  ...
  do
  {
    ...
    SoSes_01._c[rb_Index_l] = LinMsgDefault_SoSes_01[rb_Index_l]; /* VCO_219 */
    rb_Index_l++;
  } while (rb_Index_l < 8);
