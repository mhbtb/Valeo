...
static word rw_VehicleSpeed_alt = 0;
...
void GetLeWWS(void)
{
  ...
  else if(l_u8_rd_L_All_Wdw_Closed() == 1||(l_u8_rd_L_All_Wdw_Closed()== 3))
  {
  ...
  { 
    ...
    rw_VehicleSpeed_alt = l_u16_rd_L_BCM_VehicleSpeed();
    //rb_speed_in_kmh_g  = (canuint8)(l_u16_rd_L_BCM_VehicleSpeed()+(l_u16_rd_L_BCM_VehicleSpeed()>>3))/20;// N*0.05625     
    rb_speed_in_kmh_g  = (canuint8)(rw_VehicleSpeed_alt+(rw_VehicleSpeed_alt>>3))/20;// N*0.05625  
    
    if(rw_VehicleSpeed_alt > 4533)
    {
      rb_speed_in_kmh_g = 255;
    }
    else {}
    
    if(0x1FFF == rw_VehicleSpeed_alt)
    {
      rb_speed_in_kmh_g = 0;
    }  
    ...
    rb_PotiStageFromBox_g = (l_u8_rd_L_Rain_Sensitivity() > 6)?6:(l_u8_rd_L_Rain_Sensitivity());
  ...
  {                                                                     // check if system is on -> to switch off 
    ...
    {     
      ...
      else 
      {
        ...
        if ((l_u8_rd_L_All_Wdw_Closed()== 1)||(l_u8_rd_L_All_Wdw_Closed()== 3)||(rw_CommandCloseWindowTimout_mg > 6000))
        {
...
0x2：High speed "WipgSpd60Rpm";
0x3：wipe speed "WipgSpd43Rpm";
0x4：wipe speed"WipgSpd46Rpm";
0x5：wipe speed "WipgSpd50Rpm";
0x6：wipe speed "WipgSpd54Rpm";
0x7：wipe speed "WipgSpd57Rpm";
...
void RainFunction(void)
{
  ...
  byte newWiperSpeedLin_OFFtoAuto = 0;
  ...
  else
  {
    ...
    newWiperSpeedLin_OFFtoAuto = WiperSwitchChangeRequest(newWiperSpeedLin);
    
    if(newWiperSpeedLin_OFFtoAuto==1)
    {
    l_u8_wr_L_RS_WiperAutoSpReq(newWiperSpeedLin_OFFtoAuto);
    }
    else
    {
     l_u8_wr_L_RS_WiperAutoSpReq(newWiperSpeedLin);
    }
...
void LightFunction(void)
{
   ...
   l_u8_wr_L_Light_Switch_Request(rb_ReasonForLightOn_l);  
}
...
void StandByMode(void)
{
  ...
  if(rbi_Command_CloseWindows_g == TRUE)               // send close window command
  {
     ...
     l_u8_wr_L_Sunroof_Win_CloseReq(1); 
...
void OutPutBrightnessValue(void)
{
  ...
  else
  {
    ...
    else
    {
      ...
      if(rb_RSControlFlags_g == cb_ONLY_RAIN)// 只有雨功能，无HUD 20180702
      {
          l_u8_wr_L_Light_HudBrightnessValue(0);                        // FW Helligkeit Fehler
...
static void CheckErrors(void)
{
  ...
  l_bool_wr_L_RS_SolarSensorError(0);//(rb_RS_LS_Error1_g != 0)||(rb_RS_LS_Error2_g != 0)||(rb_LightS_Error1_g != 0)||(rbi_HammingError_g != 0)||(rbi_CRC_ChangedByDiagnostic_g));
}
...
byte WiperSwitchChangeRequest(byte rb_CurrSpeed)
{
    ...
    {
      if((rb_wiperStatue_old_g == 0) && (l_u8_rd_L_BCM_RS_RainSwSts() == 5) && (rb_CurrSpeed == 0))
      {
   ...
   rb_wiperStatue_old_g = l_u8_rd_L_BCM_RS_RainSwSts();//If the wiper switch change from OFF to Auto-mode the sensor shall request one single wipe.
   return rb_CurrSpeed;
}
/********************End**************************/