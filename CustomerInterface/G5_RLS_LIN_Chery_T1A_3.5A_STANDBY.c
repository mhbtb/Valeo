...
void getTransmission(byte trans_550nm,byte trans_880nm);
...
void GetLeWWS(void)
{
  ...
  byte rb_ReceivedMSG_l=0;
  byte rb_Empfindlichkeit_LS_l = 3;
  static byte rb_KL15_old_mg = 0;
#ifdef CS_STANDBY     
  if (l_u8_rd_S_WindowSts() == 2)                                                 /*Window Open*/
  {
  ...
  if(l_flg_tst_WIPESW())
  {
    ...
    rb_ReceivedMSG_l = 1;
    getTransmission(l_u8_rd_S_Transmission_550nm_Rsp(),l_u8_rd_S_Transmission_880nm_Rsp());
  }
  ...
  { 
    ...
    rb_speed_in_kmh_g  = (byte)(l_u16_rd_VehicleSpeedVSOSig() * 0.0625);
    ...
    if(l_bool_rd_S_SENS_ValidData() == 0)
    {
  ...
  {                                                                     // check if system is on -> to switch off 
   if((rw_NoCommunicationCounter_g > CW_LIN_E2_IDLE_TIMOUT))                               /***4S***/
    { 
      ...
      {                                                      //auto close windows                         
        if ((l_u8_rd_S_WindowSts() == 1) || (rw_CommandCloseWindowTimout_mg > 6000))
        {
...
{
  //static byte rb_MSG_Counter_s = 0;
  byte rb_transmission_l;
  Check_StartupActiveErrors();
   
  if(l_flg_tst_AUTOWIPE())
  {
    l_flg_clr_AUTOWIPE();
    rbi_ConnectedToMaster_g = 1; 
  }
  
  rb_transmission_l = rb_Kodierbyte2_g & 0x0f;
  if(rb_transmission_l == 0)
  {
    l_u8_wr_S_Transmission_550nm(15);
  }
  else
  { 
    l_u8_wr_S_Transmission_550nm(rb_transmission_l*5+15);
  }
  
  rb_transmission_l = (rb_Kodierbyte2_g >> 4);
  if(rb_transmission_l == 0)
  {
    l_u8_wr_S_Transmission_880nm(15);
  }
  else
  { 
    l_u8_wr_S_Transmission_880nm(rb_transmission_l*5+15);
  }
...
void RainFunction(void)
{
  ...
  byte rb_CurrentWipeSpdReqst_l = 0;
  ...
  else  
  {
    if(IsAutoWiper())
    {
      ...
      {
		  /*********HU 190212*****/
		  if(l_bool_rd_S_AUTOH() == 1)
		  {
			  rb_CurrentWipeSpdReqst_l = 2;
		  }
		  if(l_bool_rd_S_AUTOL1() == 1)
		  {
			  rb_CurrentWipeSpdReqst_l = 1;
		  }
		  if((l_bool_rd_S_AUTOH() == 0) && (l_bool_rd_S_AUTOL1() == 0))
		  {
		  	rb_CurrentWipeSpdReqst_l = 0;
		  }
		  /*********HU 190212*****/
        
         if ((rbi_RSWipeSlowDmd==0)&&(rbi_RSWipeFastDmd==0))
         {
           newWiperSpeedLin = 1; //interval mode
         }
         else if (wiperSpeed7to2(rb_CurrentWipeSpdReqst_l, rb_WiperSpeed_g, rb_wiperSpeedHysteresis) != 0)
         {
           newWiperSpeedLin = wiperSpeed7to2(rb_CurrentWipeSpdReqst_l, rb_WiperSpeed_g, rb_wiperSpeedHysteresis);  //continuous mode
         }   
      }
  ...
  {
    l_bool_wr_S_rain(((rw_rain_intensitiy_g>>9)>0) ? 1:0)
  }
...
void LightFunction(void)
{
  ...
  byte rb_LightSensError_l;
//  byte rb_ReasonForLightOn_l = 0;
  
  rb_LightSensError_l = (rb_RS_LS_Error1_g != 0)||(rb_RS_LS_Error2_g != 0)||(rb_LightS_Error1_g != 0)||(rbi_HammingError_g != 0)||(rbi_CRC_ChangedByDiagnostic_g);
 /* PRQA S 3415, 4112 1 */ /* << Getter functions must not have side effects >>*/
  if ((((((IS_LS_ANY_ERRORCOUNTER()) > 0))||((rb_LightSensError_l) > 0))||(((rbi_UnderVoltage_g) == TRUE))
  ...
  { /* PRQA S 3415 1 */ /* << Swapping expressions not feasible for performance >>*/
    ...
    {
      rb_TempLightOn_l = l_bool_rd_S_AUTOLAMP();                                                               /* stay in old light mode and do */
    }
  }
      ...
      && ((rb_RSControlFlags_g == cb_HUMIDITY_SOLAR )||(rb_RSControlFlags_g == cb_ONLY_HUMIDITY)))
  {
    ...
    l_bool_wr_S_AUTOLAMP(TRUE);   
  } 
  else
  {
    /* PRQA S 434, 436, 781 7 */ /* <<usind Micro Controller specific command > */
    DISABLE_SCIINTERRUPT();
    l_bool_wr_S_AUTOLAMP(rb_TempLightOn_l);  
    l_bool_wr_S_AUTOPANEL(rb_TempLightOn_l);  
    ENABLE_SCIINTERRUPT();       
  } 

  //l_bool_wr_LightSensorActive(1);                                                                                                                    
//  if(((rbi_LightSensorActive_g == TRUE) && (rbi_EEPROMLoad_g == FALSE)) && (rb_TempLightOn_l>0))                /**Send the reason why should lamp on**/
//  {        
...
//    } 
    ...
    {
      l_u16_wr_RLS2_Status_DewPointTemp_Value(0X7FF);
    }    
    else
    {
    ...
    if(rw_Temperature_raw_g == cw_HUMTEMP_INIT_g)
    {
      l_u8_wr_RLS2_Status_WinGlassTemp_Value(0xFF);
    }
    else
    {   
...
void StandByMode(void)
{
  ...
  {
	  l_bool_wr_S_WindowCmd(TRUE);                // send close window command
...
void OutPutBrightnessValue(void)
{
  ...
  if((rbi_LightValuesInitialised_g == FALSE)|| (rbi_EEPROMLoad_g == TRUE))
  {
    ...
    {      
      l_u8_wr_RLS_Brightness_FW(0XFF);                                   //FW invailed
      l_u16_wr_RLS_Brightness_IR(0x3FF);
      l_u8_wr_RLS2_Status_PasSideSunInt_Value(0xFF);                     //SOLAR invailed
      l_u8_wr_RLS2_Status_DriSideSunInt_Value(0xFF);                     //SOLAR invailed
    }
  }
  else
  {
    ...
    else if(l_bool_rd_RLS_Fault_Light() == TRUE)                    /*  VDR469   set for error*/
    {
      l_u16_wr_RLS_Brightness_IR(0x3FF);                                  // IR Helligkeit Fehler
      l_u8_wr_RLS_Brightness_FW(0XFF);
      l_u8_wr_RLS2_Status_PasSideSunInt_Value(0xFF);                            /* Init*/
      l_u8_wr_RLS2_Status_DriSideSunInt_Value(0xFF);                             /* Init*/
    }
    else
    {
      ...
      else
      {
        ...
        rw_FW_brightness_l = rw_FW_brightness_l / 4;
        l_u8_wr_RLS_Brightness_FW(((rw_FW_brightness_l>255)?255:rw_FW_brightness_l));        
      }
      
      if(rb_IrLightDiv8_g < 0xFD)
      {
        l_u16_wr_RLS_Brightness_IR((byte)rb_IrLightDiv8_g);
      }
      else
      {
        l_u16_wr_RLS_Brightness_IR(0xFD);     // IR Helligkeit max
      }
      ...
      if ((rbi_SoSe_Enabled_g == TRUE)/* && (TRUE == rbi_SolarValuesInitialised_g)*/)
      {
        ...
        l_u8_wr_RLS2_Status_DriSideSunInt_Value(rw_SoSe_SunInt_2D_Le_l);
        ...
        l_u8_wr_RLS2_Status_PasSideSunInt_Value(rw_SoSe_SunInt_2D_Re_l);       
      }
      else
      {
        l_u8_wr_RLS2_Status_PasSideSunInt_Value(0xFE); /* Init/nicht verfuegbar */
        l_u8_wr_RLS2_Status_DriSideSunInt_Value(0xFE); /* Init/nicht verfuegbar */
      }
#endif
...
void VoltageDetect(void)
{
  ...
  {
    l_u8_wr_RLHS_VotalgeError(1);
  }
  else if(rbi_OverVoltage_g == TRUE)
  {
    l_u8_wr_RLHS_VotalgeError(2);
  }
  else
  {
  	l_u8_wr_RLHS_VotalgeError(0);
  }    
}
...
void getTransmission(byte trans_550nm,byte trans_880nm)
{
  ...
  if((trans_550nm > 1) && (trans_880nm > 1) && (trans_550nm <100) && (trans_880nm <100))
  {
    rb_550nm_Transmission_local_Coding = ((trans_550nm) + 2) / 5; 
    rb_880nm_Transmission_local_Coding = ((trans_880nm) + 2) / 5; 
    
    if(rb_550nm_Transmission_local_Coding >= 18)
    {
      rb_550nm_Transmission_local_Coding = 18;
    }
    else if(rb_550nm_Transmission_local_Coding <= 3)
    {
      rb_550nm_Transmission_local_Coding = 3;
    }
    rb_550nm_Transmission_local_Coding = rb_550nm_Transmission_local_Coding - 3;
  
    if(rb_880nm_Transmission_local_Coding >= 18)
    {
      rb_880nm_Transmission_local_Coding = 18;
    }
    else if(rb_880nm_Transmission_local_Coding < 3)
    {
      rb_880nm_Transmission_local_Coding = 3;
    }
    rb_880nm_Transmission_local_Coding = rb_880nm_Transmission_local_Coding - 3;
...
static void CheckErrors(void)
{
  ...
  //l_bool_wr_RainSensorError((rb_RS_LS_Error1_g != 0)||(rb_RS_LS_Error2_g != 0)||(rb_RainS_Error1_g != 0) ||(rbi_HammingError_g != 0));  
  //l_bool_wr_LightSensorError((rb_RS_LS_Error1_g != 0)||(rb_RS_LS_Error2_g != 0)||(rb_LightS_Error1_g != 0)||(rbi_HammingError_g != 0)||(rbi_CRC_ChangedByDiagnostic_g));
  l_bool_wr_S_RSMfault((rb_RS_LS_Error1_g != 0)||(rb_RS_LS_Error2_g != 0)||(rb_RainS_Error1_g != 0) ||(rbi_HammingError_g != 0)||(rb_LightS_Error1_g != 0)||(rbi_CRC_ChangedByDiagnostic_g));
}
...
void ApplLinInit(byte rb_LIN_State_l)
{
  ...
  byte rb_Index_l; 
  static const byte LinMsgDefault_RLSs_01[8] = {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};   /* id: 0x011 */
  //static const byte LinMsgDefault_RLSs_02[8] = {0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0x3F};   /* id: 0x023 */
  
  rb_Index_l = 0;
  do
  {
    AUTOWIPE._c[rb_Index_l] = LinMsgDefault_RLSs_01[rb_Index_l];
    //RLHS._c[rb_Index_l] = LinMsgDefault_RLSs_02[rb_Index_l];
    rb_Index_l++;
  } while (rb_Index_l < 8);
...
__callt void ApplLinSlavePreCopy(byte rb_MessageID_l)                                     /* callt function faster */
{
  ...
  {
    l_bool_wr_Response_error(rbi_latched_ResponseError_mg);
    l_bool_wr_S_commfault(rbi_latched_ResponseError_mg);
