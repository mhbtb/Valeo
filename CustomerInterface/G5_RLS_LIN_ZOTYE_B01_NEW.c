...
/* PRQA S 3218 7 */ /* <<needed at this level for better debugging>> */
static const byte cab_AUDI_SenMapping[16] = {0,0,0,2,2,2,3,3,3,4,4,4,5,5,5,7};
/* PRQA S 3218 7 */ /* <<needed at this level for better debugging>> */
...
void GetLeWWS(void) /*Information from SCS*/
{
  ...
  static byte rb_AfterResetCounter_l;
    ...
    if(l_flg_tst_LIN_Master_BCM())               /*Master Request?*/
    {
      l_flg_clr_LIN_Master_BCM();                 /*loesche flag*/
      rbi_LightSensitivityRecieved_g = TRUE; 

      /* Park position information comes from BCM message */
      if(l_u8_rd_ParkingPosition() == 0)
      {
        rbi_RealParkContact_g = TRUE;
      }
      else if(l_u8_rd_ParkingPosition() == 1)
      {
    ...
    if(l_u8_rd_WiperSwitchPosition() == 4)   /* Waschpumpe aktiv?*/
    {
    ...
    rbi_LINRainsensorDisabled_g = FALSE;     /**No active button,always False**/

#ifdef STARTUP_WIPE   
    rd_Aussentemp_g = l_u8_rd_Aussentemp_gef();
#endif
    /***Zotye_B01_NEW had no standby function,no need to know the ignition status***/
    rbi_Ignition_g = TRUE;
    ...
    {
      l_bool_wr_RainSensorActive(1);
    }
    else
    {
      l_bool_wr_RainSensorActive(0);
    }
    
    if(l_u8_rd_WiperSwitchPosition() == 1) // copy automatic wiping mode
    {
    ...
    if(l_u8_rd_VehicleSpeed() < 63)/*Check for max value   speed faster than 200 (255km/h) */
    {
      rb_speed_in_kmh_g = (byte)((word)(l_u8_rd_VehicleSpeed())*4); /*recalibrate Data multily by 2,56*/
    }
    else
    {
      if(l_u8_rd_VehicleSpeed() < 64)
      {
    ...
    if (l_u8_rd_SensitivitySetting() <= 5) /*VDR_408 do not update if init*/
    {
      rb_PotiStageFromBoxTemp_l = cab_AUDI_SenMapping[l_u8_rd_SensitivitySetting()*3];
    ...
    {
      rb_PotiStageFromBox_g = 7;  /***if received the invalid value,then use the most sensitivity value***/
    }  
      
    rb_LIN_SensitivityShift_g =  3 + 6;       //adaptation Variable for sensitivity factor
#ifndef AUDI_B8
...
void CopyRLSData(void)
{
...
#endif
  byte newWiperSpeedLin =0;/***This is for mapping wiper_speed.***/
  ...
  if(l_flg_tst_LIN_Slave_RLS())                    // pruefen ob eigene Nachricht korrekt empfangen wurde
  {
    l_flg_clr_LIN_Slave_RLS() ;                    // loeschen des Pruefflags
         ...
         &&(rb_WiperSpeed_g == 0))
  {
    ...
    l_u8_wr_RainsensorWipeRequest(1);
  }
  ...
  else
  {
    ...
    if(((l_u8_rd_WiperSwitchPosition()==2)||(l_u8_rd_WiperSwitchPosition()==3))&&(rb_WiperSpeed_g > 1))
    {
      l_u8_wr_RainsensorWipeRequest(1);   // Wischergeschwindigkeit uebergeben
    }
    else if(l_u8_rd_WiperSwitchPosition()==1)
    {
      if(rb_WiperSpeed_g!=0)
      {
         ...
         else if (wiperSpeed7to2(l_u8_rd_RainsensorWipeRequest(), rb_WiperSpeed_g, rb_wiperSpeedHysteresis) != 0)
         {
           newWiperSpeedLin = wiperSpeed7to2(l_u8_rd_RainsensorWipeRequest(), rb_WiperSpeed_g, rb_wiperSpeedHysteresis);  //continuous mode
         }   
       }
      else
      {
      ...
      l_u8_wr_RainsensorWipeRequest(newWiperSpeedLin);   // Wischergeschwindigkeit uebergeben
  ...
  else
  {
    ...
    l_bool_wr_RainSensorError(((rb_RS_LS_Error1_g&0x3) != 0)||(TRUE==bi_RS_VariantError_g)||(rbi_HammingError_g != 0));
  }
  ...
  if ((((((IS_LS_ANY_ERRORCOUNTER()) > 0))||((l_bool_rd_LightSensorError()) > 0))||(((rbi_UnderVoltage_g) == TRUE))
  ...
  if(l_bool_rd_LightSensorError() == TRUE)
  {
    rb_TempLightOn_l = 1;
  }
  /*VCO_50*//*VDR_129 use Sensitivity Treatment*/
  else if(((uF_modi_lightsens_g.AllBit & 0x2F)>0)||(rbi_InternalResetOccured_g > 0))//||(rbi_Regulation48LightOnFlag_g !=0)) xsy 20180813 remove 
  {
  ...
  { /* PRQA S 3415 1 */ /* << Swapping expressions not feasible for performance >>*/
    ...
    {
      rb_TempLightOn_l = l_bool_rd_HeadLampsOn() ; /* stay in old light mode and do */
    }
  }
      ...
      && ((rb_RSControlFlags_g == cb_HUMIDITY_SOLAR )||(rb_RSControlFlags_g == cb_ONLY_HUMIDITY)))
  {
    ...
    l_bool_wr_HeadLampsOn(TRUE);                     
  } 
  else
  {
    /* PRQA S 434, 436, 781 7 */ /* <<usind Micro Controller specific command > */
    DISABLE_SCIINTERRUPT();
    l_bool_wr_HeadLampsOn(rb_TempLightOn_l);                      // Licht ein = TRUE
    /* PRQA S 434, 436, 781 7 */ /* <<usind Micro Controller specific command > */
    ENABLE_SCIINTERRUPT();
  }
  
  l_bool_wr_TunnelDetection(rbi_modi_lightsens_tunnel_g == TRUE);
  l_bool_wr_SmallLampsOn(rbi_modi_lightsens_tunnel_g == TRUE);
  
  if ((l_bool_rd_RainSensorError()==FALSE) && ((rbi_WindowOpenIgnitionWasOn_g == TRUE) && (rbi_Ignition_g== FALSE)) &&((rb_Verglasun_offenDebounce_mg > cb_VERGLSUNG_OPEN_DELAY_mg)&& (rbi_StandbyFunction_enabled_g == TRUE)))
  {
  ...
  {
    l_bool_wr_OverVolFailSts(1);                  // Overvoltage
  }
  else
  {
    l_bool_wr_OverVolFailSts(0); 
  }
  ...
  else
  {
    ...
    else if(l_bool_rd_LightSensorError() == TRUE) /*  VDR469   set for error*/
    {
...
void ApplLinInit(byte rb_LIN_State_l)
{
  ...
  do
  {
    LIN_Slave_RLS._c[rb_Index_l] = LinMsgDefault_RLSs_01[rb_Index_l];
    rb_Index_l++;
  } while (rb_Index_l < 2);
...
__callt void ApplLinSlavePreCopy(byte rb_MessageID_l)    /* callt function (faster */
{
  ...
  {
    l_bool_wr_RLSCommErrorSts(rbi_latched_ResponseError_mg);
