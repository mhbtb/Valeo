...
void GetLeWWS(void)
{
  ...
  if(l_flg_tst_Master_BCM01())
  { 
    l_flg_clr_Master_BCM01();
...
void CopyRLSData(void)
{
  ...
  if(l_flg_tst_Slaver_RLS01())
  {
    l_flg_clr_Slaver_RLS01() ;
    rbi_ConnectedToMaster_g = 1; 
  }
...
void RainFunction(void)
{
  ...
  else  
  {
    if(IsAutoWiper())
    {
      ...
      l_u8_wr_RainsensorWipeRequest(newWiperSpeedLin);    
    }
    else{}   
...
//  byte rb_ReasonForLightOn_l = 0;
 ...
 /* PRQA S 3415, 4112 1 */ /* << Getter functions must not have side effects >>*/
  ...
  if(l_bool_rd_LightSensorError() == TRUE)                                                                                 //sensor in error
  {
  ...
  else
  {
    ...
    l_bool_wr_HeadLampsOn(rb_TempLightOn_l);  
    l_bool_wr_SmallLampsOn(rbi_modi_lightsens_tunnel_g == TRUE);
    ENABLE_SCIINTERRUPT();       
  }  
  l_bool_wr_LightSensorActive(1);
  l_bool_wr_TunnelDetection(rbi_modi_lightsens_tunnel_g == TRUE);                                                                                                                       /**Send the reason why should lamp on**/
