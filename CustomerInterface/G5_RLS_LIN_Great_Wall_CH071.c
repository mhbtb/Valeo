...
void GetLeWWS(void) /*Information from SCS*/
{
  ...
  static unsigned char rbi_Klemme_15_l; /***used for KL_15***/
  static byte rb_delay_l; 
  static unsigned char rbi_RLS_Sensitivity_l;

/***Great Wall had Auto_Close_Window function 2018_04_01 Blake  Always open this function***/   
  if (l_u8_rd_SrPositionBCM() == 6)   /**Not fully close means open Confirm with Greatwall**/
  {
...
/*************************************End 2018_04_01 Blake********************************/
  if(l_flg_tst_BCM_11())               /*Master Request?*/
  {
    l_flg_clr_BCM_11();                 /*loesche flag*/
    rbi_LightSensitivityRecieved_g = TRUE; 

    rbi_RealParkContact_g = l_bool_rd_WiperParkPosition();  /***Match Ldf for GreatWall***/
    
    if((l_u8_rd_WiperSwitchSts() == 4) || (l_u8_rd_WiperSwitchSts() == 5))   /* Waschpumpe aktiv?*/
    {
    ...
    rbi_LINRainsensorDisabled_g = FALSE;      /**No HMI for Rain function to Enable or Disable this feature**/
   
    
    if(l_u8_rd_SystemPowerMod() > 0)
    {
      if(rb_delay_l <= 5)
      {
    	  rb_delay_l++;
      ...
      {
    	  rbi_Klemme_15_l = TRUE;
      }
    }
    else
    {
    	rb_delay_l = 0;
    	rbi_Klemme_15_l = FALSE;
    }
      
    /* ignore ignition off if speed higher 4km/h*/
    if((rbi_Klemme_15_l == TRUE)||(rb_speed_in_kmh_g > cb_IGNORE_IGNITION_SPEED)) /*VCO_083*/
    {
      rbi_Ignition_g = TRUE;
      ...
      if(l_u8_rd_WiperSwitchSts()== 1) // copy automatic wiping mode
      {
...
/*****2018_04_01 Blake_Li *****/    
    if(l_u16_rd_VehicleSpeed() < 5334)/*Check for max value   speed faster than 200 (255km/h) */
    {
      rb_speed_in_kmh_g = (byte)((word)(l_u16_rd_VehicleSpeed()) * 0.056); /*recalibrate Data multily by 2,56*/
    }
    else
    {
    ...
    rbi_RLS_Sensitivity_l = l_u8_rd_RLSSensitivityReq() * 3 + 1;  
    rb_PotiStageFromBoxTemp_l = cab_AUDI_SenMapping[rbi_RLS_Sensitivity_l];
  ...
  {    // check if system is on -> to switch off
    ...
    if((rw_NoCommunicationCounter_g > CW_LIN_E2_IDLE_TIMOUT))
    {
      ...
      {
        if ((l_u8_rd_SrPositionBCM()==3)||(rw_CommandCloseWindowTimout_mg > 6000))   /***If window close or command send over 1min***/
        {
...
void GetLsWWS(void)
{
  ...
  if(l_flg_tst_BCM_12())                  //Wischermotor request?
  {
    l_flg_clr_BCM_12();                  //loesche flag   
    rbi_ConnectedToMaster_g = 1;                   // set connected to Master
   
    /***For light transmission value***/
...
void CopyRLSData(void)
{
  ...
  else
  {
    ...
    {      
      if(rb_WiperSpeed_g!=0)
      {
        ...
        else if (wiperSpeed7to2(l_u8_rd_WiperReq(), rb_WiperSpeed_g, rb_wiperSpeedHysteresis) != 0)
        {
          newWiperSpeedLin = wiperSpeed7to2(l_u8_rd_WiperReq(), rb_WiperSpeed_g, rb_wiperSpeedHysteresis);  //continuous mode
        }   
      }
      else
      {
    ...
    l_u8_wr_WiperReq(newWiperSpeedLin);   // Wischergeschwindigkeit uebergeben 
  ...
  if((TRUE==rbi_RainSensor_enabled_g) && (FALSE==rbi_RainsensorDisabled_g))
  {
    ...
    /*SM240*/
    l_bool_wr_RLSErrorSts((rb_RS_LS_Error1_g != 0)||(rb_RS_LS_Error2_g != 0)||(rb_RainS_Error1_g != 0) ||(rbi_HammingError_g != 0));
  }
  else
  {
    /*only RAM/ROM error and RS version error*/
    l_bool_wr_RLSErrorSts(((rb_RS_LS_Error1_g&0x3) != 0)||(TRUE==bi_RS_VariantError_g)||(rbi_HammingError_g != 0));
  }
  ...
  l_bool_wr_HardwareFailSts((rb_RS_LS_Error1_g != 0)||(rb_RS_LS_Error2_g != 0)||(rb_RainS_Error1_g != 0)||(rb_LightS_Error1_g != 0));
  
  if(rb_RS_LS_Error2_g & 0x08)
  {
    l_bool_wr_OpticalInitializationFailSts(TRUE);
  }
  else
  {
   l_bool_wr_OpticalInitializationFailSts(FALSE);
  }
  ...
  if(l_bool_rd_LightSensorFailSts() == TRUE)
  {
    rb_TempLightOn_l = 1;
  }
  ...
  { /* PRQA S 3415 1 */ /* << Swapping expressions not feasible for performance >>*/
    ...
    {
      rb_TempLightOn_l = ((l_bool_rd_LowBeamReq()) | (l_bool_rd_PositionLightReq())); /* stay in old light mode and do */
    }
  }
      ...
      && ((rb_RSControlFlags_g == cb_HUMIDITY_SOLAR )||(rb_RSControlFlags_g == cb_ONLY_HUMIDITY)))
  {
    ...
    l_bool_wr_LowBeamReq(TRUE);   
    l_bool_wr_PositionLightReq(TRUE);
  } 
  else
  {
    /* PRQA S 434, 436, 781 7 */ /* <<usind Micro Controller specific command > */
    DISABLE_SCIINTERRUPT();
    l_bool_wr_LowBeamReq(rb_TempLightOn_l);                      // Licht ein = TRUE
    l_bool_wr_PositionLightReq(rb_TempLightOn_l);               // Licht eine invers = FALSE
    /* PRQA S 434, 436, 781 7 */ /* <<usind Micro Controller specific command > */
    ENABLE_SCIINTERRUPT();
  }
 
  if(rbi_modi_lightsens_tunnel_g)
  {
   l_bool_wr_TunnelDetection(1);
  }
  else
  {
   l_bool_wr_TunnelDetection(0);
  }
/*******************Standby mode  2018_04_01   Blake*****************************/  
  ...
  {
    l_u16_wr_CloseSRReq(1);                // setzen von Verglasung Schliessen
  ...
  else
  {
    ...
    l_u16_wr_CloseSRReq(0);             // setzen von Verglasung Schliessen
  }
 /********************************************End*********************************************************/
 if(((rb_InternalTemperature_g > 0) && (rb_InternalTemperature_g < cb_TEMP_uC_AD65DEGREES_THRS_g))  || ((rw_VTEMP_ASIC_g > 0) && (rw_VTEMP_ASIC_g < cb_TEMP_ASIC_AD65DEGREES_THRS_g)))
  {
    l_bool_wr_TemperatureFailSts(TRUE);
  }
  else
  {
    l_bool_wr_TemperatureFailSts(FALSE);
  }
   
  if((rbi_UnderVoltage_g == TRUE) ||(rbi_OverVoltage_g == TRUE))
  {
    l_bool_wr_VoltageFailSts(TRUE);
  }
  else
  {
    l_bool_wr_VoltageFailSts(FALSE);
  }
...
void ApplLinInit(byte rb_LIN_State_l)
{
  ...
  byte rb_Index_l;
  static const byte LinMsgDefault_RLSs_01[8] = {0,0,0}; /* id: 0x023 "RLSs_01" */  
  rb_Index_l = 0;
  do
  {
...
__callt void ApplLinSlavePreCopy(byte rb_MessageID_l)    /* callt function (faster */
{
  ...
  {
    l_bool_wr_RainLightSensorActiveSts(!rbi_latched_ResponseError_mg);
