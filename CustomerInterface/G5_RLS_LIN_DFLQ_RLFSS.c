...
static unsigned char rb_RLS_msgCounter_g=0;
...
void GetLeWWS(void)
{
  ...
  { 
    ...
    rbi_RealParkContact_g = l_bool_rd_BCM_ParkPosition();            // wiper position
    rbi_LightSensitivityRecieved_g = TRUE;
    
    rb_speed_in_kmh_g  = ((l_u16_rd_BCM_SPD_Vehicle())+((l_u16_rd_BCM_SPD_Vehicle())>>3))/20;// N*0.05625      
    if((l_u16_rd_BCM_SPD_Vehicle()) > 4533)
    {
    ...
    rb_KL15_mg=((l_u8_rd_BCM_Status_IGN() > 0)?1:0);                            
    ...
    {                                                                    /* ignore ignition off if speed higher 4km/h*/
      ...
      {
        if((l_bool_rd_BCM_CMD_AutoWiper() == 1)                         //automatic
      ...
      if(l_bool_rd_BCM_CMD_AutoWiper() == 1)                            // copy automatic wiping mode
      {
    ...
    rb_PotiStageFromBox_g = ((l_u8_rd_BCM_RainSensitivity() > 6)?6:(l_u8_rd_BCM_RainSensitivity()));
    
    if (l_bool_rd_BCM_RQ_FrontWash())                                 /* Washpump activ? 0x5:Front washing*/
    {
...
{  
  ...
  if(    ((l_bool_rd_BCM_CMD_AutoWiper() != 1)    //auto wiper
...
void CopyRLSData(void)
{
  ...
  l_u8_wr_RLS_MsgCounter(rb_MSG_Counter_s);
...
void RainFunction(void)
{
  ...
  {
    if(l_bool_rd_BCM_CMD_AutoWiper() == 1)
    {
      if(rb_WiperSpeed_g!=0)
      {
         ...
         else if (wiperSpeed7to2(l_u8_rd_RLS_RQ_WiperSPD(), rb_WiperSpeed_g, rb_wiperSpeedHysteresis) != 0)
         {
           newWiperSpeedLin = wiperSpeed7to2(l_u8_rd_RLS_RQ_WiperSPD(), rb_WiperSpeed_g, rb_wiperSpeedHysteresis);  //continuous mode
         }   
      }
      else
      {
    ...
    {
      if((rb_wiperStatue_old_g == 0) && (l_bool_rd_BCM_CMD_AutoWiper() == 1) && (rb_WiperSpeed_g == 0))
      {
        rb_wipeForHanderChanged = 1;
        rb_wipeForHanderChangedCounter = 100;
      }
      
      if((rb_wipeForHanderChanged == 1) && (rbi_ParkPosition_g == TRUE) && (rb_wipeForHanderChangedCounter > 0) && (rb_WiperSpeed_g == 0) && ((rb_KL15_mg) == TRUE))
      {
    ...
    l_u8_wr_RLS_RQ_WiperSPD(newWiperSpeedLin);                     // Wischergeschwindigkeit uebergeben
  ...
  {
    rb_RLS_msgCounter_g = 3;                                      // for sending wiper request at least 4 times
  }
  ...
  rb_wiperStatue_old_g = ((l_bool_rd_BCM_CMD_AutoWiper()) == 1);
...
//  byte rb_ReasonForLightOn_l = 0;
 ...
 /* PRQA S 3415, 4112 1 */ /* << Getter functions must not have side effects >>*/
  if ((((((IS_LS_ANY_ERRORCOUNTER()) > 0))||((l_bool_rd_RLS_Fault_Light()) > 0))||(((rbi_UnderVoltage_g) == TRUE))
  ...
  if(l_bool_rd_RLS_Fault_Light() == TRUE)                                                                                 //sensor in error
  {
  ...
  { /* PRQA S 3415 1 */ /* << Swapping expressions not feasible for performance >>*/
    ...
    {
      rb_TempLightOn_l = l_bool_rd_RLS_RQ_LowBeam();                                                                      /* stay in old light mode and do */
    }
  } 
      ...
      && ((rb_RSControlFlags_g == cb_HUMIDITY_SOLAR )||(rb_RSControlFlags_g == cb_ONLY_HUMIDITY)))
  {
    ...
    l_bool_wr_RLS_RQ_LowBeam(TRUE);   
    l_bool_wr_RLS_RQ_PositionLamp(TRUE);
  } 
  else
  {
    /* PRQA S 434, 436, 781 7 */ /* <<usind Micro Controller specific command > */
      DISABLE_SCIINTERRUPT();
      l_bool_wr_RLS_RQ_LowBeam(rb_TempLightOn_l); 
      l_bool_wr_RLS_RQ_PositionLamp(rb_TempLightOn_l);
      ENABLE_SCIINTERRUPT();
  }
...
//    } 
    ...
    {
      if (rw_DewPoint_g <= 1000)                /* dew point should be less than or equal to Scheibe temperature */
      {                                                                       
        l_u16_wr_RLS2_Status_DewPointTemp_Value(rw_DewPoint_g);         
      }
      else
      {
        l_u16_wr_RLS2_Status_DewPointTemp_Value(1000);
      }
    }
    ...
    { 
      if(rb_Humidity_g<=200)
      {
        l_u8_wr_RLS2_Status_RelativeHumidity_Lvl(rb_Humidity_g);
      }
      else
      {
        l_u8_wr_RLS2_Status_RelativeHumidity_Lvl(200);
      }
    }
   
                                                              /*the temperature of inter sensor , not screen  range -39.5~87.5*/
    if(rw_Temperature_raw_g < 0x27b) /* The temperature less than -39,5 the tool doesnot support */
    {
      rw_Temperature_Sensor_g = 0x27b;    
    }
    ...
    {   
      if(((rw_Temperature_Sensor_g-635)/50) < 0xfe)
      {        
        l_u8_wr_RLS2_Status_WinGlassTemp_Value(((rw_Temperature_Sensor_g-635)/50));
      }
      else
      {
        l_u8_wr_RLS2_Status_WinGlassTemp_Value(0xfe);
      }
    }        
  }
}
...
void VoltageDetect(void)
{
  ...
  {
      //l_u8_wr_L_Err_State_Over_Volt(rbi_OverVoltage_g);  //voltage error statue
  }
  else{}    
}
...
void OutPutBrightnessValue(void)
{
  ...
  if((rbi_LightValuesInitialised_g == FALSE)|| (rbi_EEPROMLoad_g == TRUE))
  {
    ...
    {      
      ...
      l_u8_wr_RLS2_Status_PasSideSunInt_Value(0xFE);                     //SOLAR invailed
      l_u8_wr_RLS2_Status_DriSideSunInt_Value(0xFE);                     //SOLAR invailed
    }
  }
  else
  {
    ...
    else
    {
      ...
      else
      {
        ...
        l_u8_wr_RLS_Brightness_FW(((rw_FW_brightness_l >= 254)?254:rw_FW_brightness_l));        
      }
      ...
      if ((rbi_SoSe_Enabled_g == TRUE)/* && (TRUE == rbi_SolarValuesInitialised_g)*/)
      {
        ...
        rw_SoSe_SunInt_2D_Le_l=((rw_SoSe_SunInt_2D_Le_l >= 0xC8) ? 0xC8 : rw_SoSe_SunInt_2D_Le_l);
        l_u8_wr_RLS2_Status_DriSideSunInt_Value(rw_SoSe_SunInt_2D_Le_l);
        
        rw_SoSe_SunInt_2D_Re_l=(rw_SoSe_SunInt_2D_Re_l >> 1);
        rw_SoSe_SunInt_2D_Re_l=((rw_SoSe_SunInt_2D_Re_l >= 0xC8) ? 0xC8 : rw_SoSe_SunInt_2D_Re_l);
...
static void CheckErrors(void)
{
  ...
  l_bool_wr_RLS_Fault_Rain((rb_RS_LS_Error1_g != 0)||(rb_RS_LS_Error2_g != 0)||(rb_RainS_Error1_g != 0) ||(rbi_HammingError_g != 0));  
  l_bool_wr_RLS_Fault_Light((rb_RS_LS_Error1_g != 0)||(rb_RS_LS_Error2_g != 0)||(rb_LightS_Error1_g != 0)||(rbi_HammingError_g != 0)||(rbi_CRC_ChangedByDiagnostic_g));
 }
...
void ApplLinInit(byte rb_LIN_State_l)
{
  ...
  byte rb_Index_l; 
  static const byte LinMsgDefault_RLSs_01[8] = {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};   /* id: 0x023 */
  static const byte LinMsgDefault_RLSs_02[8] = {0x00,0x00,0x00,0xf8,0xfb,0x03,0x00,0x00};   /* id: 0x011 */
  
  rb_Index_l = 0;
  do
  {
    RLS_01._c[rb_Index_l] = LinMsgDefault_RLSs_01[rb_Index_l];
...
__callt void ApplLinSlavePreCopy(byte rb_MessageID_l)                                     /* callt function faster */
{
  ...
  {
    l_bool_wr_RLS_REP_Error(rbi_latched_ResponseError_mg);
