...
--                             is simpley created from this char array, rather
--                             than from stdin.  By deafult, "printf" is used
...
/* -------------------------------------------------------------------------
-- defininition for stdin data size type
----------------------------------------------------------------------------*/
...
/* The below pointer points to begining of FileTable */
...
*************************************************************************/
void  SetFileOffset(struct FileData * fileData, unsigned int currentOffset)
{
  unsigned int lindex;
  /* Get The index into Current Offset Table for Files */
...
/**************************************************************************
Function: AddFileOffSet
Parameters: fileData - the FileData structure to get a line from
            Current Offset- Offset Within a file
Description: This function sets current offset within a file. 
*************************************************************************/
void  AddFileOffset(struct FileData * fileData, unsigned int currentOffset)
{
  unsigned int lindex;
  /* Get The index into Current Offset Table for Files */
  lindex = fileData - vcast_file_table;
...
line pointer to the beginning of the file contents.
  *************************************************************************/
void ResetCurrentLine(struct FileData * fileData){
  ...
  if (fileData != (struct FileData *)NULL) {
    ...
    /* Reset offset  withing file to zero */
 ...
 *************************************************************************/
void AddNewLine(struct FileData * fileData, char * lineContents){
  int lindex;
  unsigned int lCurrentOffset ;
  unsigned int lInitialOffset ;
  unsigned int offset ;
  ...
  /* Get The index into Current Offset Table for Files */
  lindex = fileData - vcast_file_table;
  /* Total Offset  = Initial Offset in the TOC + Offset within the file */
  offset = vcast_file_offsets[lindex] +  fileData->offset;
  /* Add the line to input Data Array at offset computed above */
  AddLineToInputDataArray(lineContents, lineLen, offset);
...
struct FileData * GetFileFromName(char * fileName){
  ...
  int index = 0;
  /* Iterate from begining to end for matching file name */
...
/* ------------------------------------------------------------------------------- */
void vectorcast_longest_int_to_str(VCAST_LONGEST_INT vcast_value, char *vcast_str)
{
  vcast_signed_to_string ( vcast_str, vcast_value );
}
...
if (vcastFirstReadOfstdin) {
   ...
   while ((len=VCAST_strlen(currentLine)) > 0){
     ...
     if (VCAST_strncmp(currentLine, (char *)VCAST_INPUT_ENDING_MARKER, VCAST_strlen(VCAST_INPUT_ENDING_MARKER)) == 0){
        AddNewLine(currentFile, currentLine);
       break;
     }
     ...
     if (StartsNewFile(currentLine)) {
          ...
          AddFileOffset(currentFile, len + vcastLengthBytes);
     }
     else {
        if (currentFile != 0){
           ...
           AddNewLine(currentFile, currentLine);
           /* Update offset for  file */
           AddFileOffset(currentFile, len + vcastLengthBytes+1);
        } 
     }
...
void vcast_float_to_string ( char *mixed_str, vCAST_long_double f ) {
   ...
   vcast_signed_to_string ( mixed_str, f );

#elif defined(VCAST_FP) || defined(VCAST_NO_SPRINTF)

   /* if we can't use sprintf, use our internal function */
   vectorcast_float_to_string ( mixed_str, f );

#else
#if VCAST_HAS_FLOAT128 && VCAST_HAS_QUADMATH
   quadmath_snprintf(mixed_str, VCAST_MAX_STRING_LENGTH, VCAST_FLOAT_FORMAT, f);
#else
   /* otherwise just use sprintf */
   sprintf ( mixed_str, VCAST_FLOAT_FORMAT, f );
