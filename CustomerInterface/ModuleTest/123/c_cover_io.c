...
#ifdef VCAST_USE_STATIC_MEMORY
extern AVLTree VCAST_ATTRIBUTE_CODE vcast_avltree_data_pool[];
int vcast_avltree_data_pos = 0;
#endif /* VCAST_USE_STATIC_MEMORY */

AVLTree *vcast_avltree_new(int vcast_size);
AVLTree *
...
{
  int unique_id;
  for (unique_id = 0; unique_id < vcast_mcdc_bytes; ++unique_id)
  {
...
VCAST_DUMP_COVERAGE_DATA_ID(void)
{
  ...
  for (vcast_unit_cur = vcast_unit_list_values; vcast_unit_cur->vcast_unit; vcast_unit_cur++) {
    ...
    VCAST_DUMP_MCDC_COVERAGE_DATA_ID(vcast_unit, vcast_unit_cur->vcast_mcdc_array,
                                     vcast_unit_cur->vcast_mcdc_bytes);
#endif /* #ifdef VCAST_USE_STANDARD_MCDC_INSTRUMENTATION */
...
#if defined (VCAST_COVERAGE_TYPE_MCDC) || defined(VCAST_COVERAGE_TYPE_STATEMENT_MCDC)
#ifdef VCAST_USE_STANDARD_MCDC_INSTRUMENTATION
    VCAST_CLEAR_BUFFER ( (char*)vcast_unit_cur->vcast_mcdc_array, vcast_unit_cur->vcast_mcdc_bytes * sizeof(AVLTree) );
#endif /* #ifdef VCAST_USE_STANDARD_MCDC_INSTRUMENTATION */
#ifdef VCAST_USE_OPTIMIZED_MCDC_INSTRUMENTATION
#if VCAST_OPTIMIZED_MCDC_STORAGE_THRESHOLD >= 0
...
void vectorcast_float_to_string( char *mixed_str, vCAST_long_double f ) {
   ...
   vCAST_long_double f_abs;  /* the absolute value of f */
   ...
   if ( VCAST_special_compare ( (char *)&f, (char *)&vcNAN, vcDoubleSize ) ) {
      vectorcast_strcpy ( mixed_str, "NaN" );
      return;
   }

   if ( VCAST_special_compare ( (char *)&f, (char *)&vcNINF, vcDoubleSize ) ) {
      vectorcast_strcpy ( mixed_str, "-INF" );
      return;
   }

   if ( VCAST_special_compare ( (char *)&f, (char *)&vcPINF, vcDoubleSize ) ) {
      vectorcast_strcpy ( mixed_str, "+INF" );
      return;
   }
...
/* --- determine sign of f --- */
   ...
   if( f == 0.0 ) {
      ...
      sign = 1;   /* f is positive */

   } else {
      sign = -1;  /* f is negative */
   }
...
/* --- clean up and build mixed_str --- */
   ...
   if( exp!=0 ) {
      vcast_int_to_string ( whole_str, (VCAST_SIGNED_CONVERSION_TYPE) exp );
      vcast_strcat ( mixed_str, "e" );
      vcast_strcat ( mixed_str, (exp<0)?"":"+" );
      vcast_strcat ( mixed_str, whole_str );
   }
...
void vcast_probe_print_float (vCAST_long_double f)
{
   char local_string[32];
#if !defined(VCAST_UNIT_TEST_TOOL) || defined(VCAST_FP) || defined(VCAST_NO_SPRINTF)
   vectorcast_float_to_string(local_string, f);
#else
   sprintf ( local_string, VCAST_FLOAT_FORMAT, f );
#endif /* !VCAST_UNIT_TEST_TOOL || VCAST_FP || VCAST_NO_SPRINTF */
   vcast_probe_print (local_string);
}
#endif
