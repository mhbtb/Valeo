...
/* 15.11.10  VCO_083     AL      ignore ignition off if  is higher 4km/h                                   */
...
/* PRQA S 3218 7 */ /* <<needed at this level for better debugging>> */
//static const byte cab_AUDI_SenMapping[16] = {0,0,0,2,2,2,2,2,3,3,3,4,4,5,5,3};
static const byte cab_AUDI_SenMapping[16] = {5,5,5,4,4,4,3,3,3,2,2,2,0,0,0,0};
/* PRQA S 3218 7 */ /* <<needed at this level for better debugging>> */
...
void GetLeWWS(void) /*Information from SCS*/
{
  ...
  if(l_flg_tst_LIN_Master_BCM2())               /*Master Request?*/
  {
    l_flg_clr_LIN_Master_BCM2();                 /*loesche flag*/
    rbi_LightSensitivityRecieved_g = TRUE;
    /* Park position information comes from BCM message */
    rbi_RealParkContact_g = (!l_bool_rd_F_Wip_AS());

    if(l_bool_rd_Wash_Act() == TRUE)   /* Waschpumpe aktiv?*/
    {
    ...
    if((rbi_Klemme_15_g == TRUE)||(rb_speed_in_kmh_g > cb_IGNORE_IGNITION_SPEED)) /*VCO_083*/
    {
      ...
      {
        if(((l_bool_rd_RS_Auto_Mode_Act() != FALSE)
      ...
      if(l_bool_rd_RS_Auto_Mode_Act()== TRUE) // copy automatic wiping mode
      {
    ...
    if(l_u16_rd_VehSpeed() < 2047)/*Check for max value   speed faster than 200 (255km/h) */
    {
      rb_speed_in_kmh_g = (byte)(((word)(l_u16_rd_VehSpeed()))>>3); /*recalibrate Data multily by 2,56*/
    }
    else
    {
      if(l_u16_rd_VehSpeed() == 2047)
      {
    ...
    if (l_u8_rd_RS_SEN() > 0) /*VDR_408 do not update if init*/
    {
      if(l_u8_rd_RS_SEN() > 5){
    	  rb_RS_sensitivity_g = 15;
      }else{
    	  rb_RS_sensitivity_g = 3*l_u8_rd_RS_SEN()-1;
      }
...
void GetLsWWS(void)
{
  ...
  rb_Wischer_Parklage_l = (!l_bool_rd_F_Wip_AS());
...
void CopyRLSData(void)
{
  ...
  if(l_flg_tst_LIN_Slave_RLS1())                            // pruefen ob eigene Nachricht korrekt empfangen wurde
  {
    l_flg_clr_LIN_Slave_RLS1() ;                          // loeschen des Pruefflags
  ...
  else
  {
    ...
    {      
      if(rb_WiperSpeed_g!=0)
      {
         ...
         else if (wiperSpeed7to2(l_u8_rd_Wip_Speed(), rb_WiperSpeed_g, rb_wiperSpeedHysteresis) != 0)
         {
          newWiperSpeedLin = wiperSpeed7to2(l_u8_rd_Wip_Speed(), rb_WiperSpeed_g, rb_wiperSpeedHysteresis);  //continuous mode
         }   
       }
      else
      {
    ...
    l_u8_wr_Wip_Speed(newWiperSpeedLin);   // Wischergeschwindigkeit uebergeben
  ...
  else if(((uF_modi_lightsens_g.AllBit & 0x2F)>0)||(rbi_InternalResetOccured_g > 0)||(rbi_Regulation48LightOnFlag_g !=0))
  {
    ...
    l_bool_wr_Twilight_On(TRUE);             // set bit for darkness
  }
  ...
  {
    rb_TempLightOn_l = TRUE;
    l_bool_wr_Twilight_On(FALSE);                                                 // clear bit for darkness
  }
  else
  { /* PRQA S 3415 1 */ /* << Swapping expressions not feasible for performance >>*/
    ...
    {
      rb_TempLightOn_l = l_bool_rd_Light_On() ; /* stay in old light mode and do */
    }
    l_bool_wr_Twilight_On(FALSE);                                                  // clear bit for darkness
  }
  ...
  if((rbi_LightValuesInitialised_g == FALSE)|| (rbi_EEPROMLoad_g == TRUE))
  {
    ...
    {
      l_u16_wr_IR_Bright(2046);     // IR Helligkeit Init/nicht verfuegbar
      l_u16_wr_FW_Bright(2046);  // FW Helligkeit Init/nicht verfuegbar
    }
  }
  else
  {
    ...
    if(rb_RSControlFlags_g == cb_ONLY_HUMIDITY)
    {
      /*Incase of only Feuchte and solar frame preset then set the light ON, FW and IR to init value for safety reason*/
    	...
    	l_u16_wr_FW_Bright(2046);  /* Init/nicht verfuegbar */
        /* Set IR to init value in case of only humidity sensor */
    	l_u16_wr_IR_Bright(2046);     /* Init/nicht verfuegbar */
    }
    else if(l_bool_rd_LS_Err() == TRUE) /*  VDR469   set for error*/
    {
    	l_u16_wr_IR_Bright(2047);     // IR Helligkeit Fehler
        l_u16_wr_FW_Bright(2047);  // FW Helligkeit Fehler
    ...
    else
    {
      ...
      {
        if(get_rw_FW_light16bit_g() < 2045) /*VCO_031 correct variable*/
        {
        	l_u16_wr_FW_Bright(get_rw_FW_light16bit_g());
        }
        else
        {
        	l_u16_wr_FW_Bright(2045);   // FW Helligkeit max
      ...
      if (get_rw_IR_light16bit_g() < 2047)         /* higher than limit of high value?*/
      {
        rw_IrLightDiv11_g = get_rw_IR_light16bit_g();       /*copy for LS-Application*/
      }
      else
      {
        rw_IrLightDiv11_g = 2047;                        /* set to max-value*/
      }
    
      if(rw_IrLightDiv11_g < 2045)
      {
    	  l_u16_wr_IR_Bright(rw_IrLightDiv11_g);
      }
      else
      {
    	  l_u16_wr_IR_Bright(2045);     // IR Helligkeit max
      }
...
void ApplLinInit(byte rb_LIN_State_l)
{
  ...
  byte rb_Index_l;
  static const byte LinMsgDefault_RLSs_01[7] = {0,0,0,0,0,0,0};    /* 0x14"LIN_Slave_RLS1" */

  rb_Index_l = 0;
  do
  {
    LIN_Slave_RLS1._c[rb_Index_l] = LinMsgDefault_RLSs_01[rb_Index_l];
    rb_Index_l++;
  } while (rb_Index_l < 7);
...
__callt void ApplLinSlavePreCopy(byte rb_MessageID_l)    /* callt function (faster */
{
  ...
  {
    l_bool_wr_LIN_Comm_Err(rbi_latched_ResponseError_mg);
