...
**/
  ...
  if (l_bool_rd_bcm_rlsAutomaticModeActive())                                                 /*Window Open*/
  {
  ...
  if(l_flg_tst_BCM_LIN2_state())
  { 
    l_flg_clr_BCM_LIN2_state();
    rw_NoCommunicationCounter_g = 0;                                  // reset no communication counter
    rbi_ConnectedToMaster_g = 1;   
    rbi_RealParkContact_g = l_bool_rd_bcm_frontWiperParkingSts();                       // true=in park position
    rbi_LightSensitivityRecieved_g = TRUE;                            
        
    rb_speed_in_kmh_g  = (l_u16_rd_bcm_vehicleSpeed()+(l_u16_rd_bcm_vehicleSpeed()>>3))/20;// N*0.05625      
    if((l_u16_rd_bcm_vehicleSpeed())> 4533)
    {
      rb_speed_in_kmh_g = 255;
    }
    else {}
   
    Curr_speed = l_u16_rd_bcm_vehicleSpeed();
    if(Curr_speed == 8191)
    {
      rb_speed_in_kmh_g = 0;
    }
    
    if(l_bool_rd_bcm_vehicleSpeedValid() == 0)
    {
      rb_speed_in_kmh_g = 0;
    }
    else
    ...
    rb_KL15_mg = ((l_u8_rd_bcm_powerMode() == 3)?1:0)  ;     
    ...
    {                                                                    /* ignore ignition off if speed higher 4km/h*/
      ...
      {
        if((l_u8_rd_bcm_frontWiperStatus()==5)                                              //automatic
      ...
      if(l_u8_rd_bcm_frontWiperStatus()==5)                                                // copy automatic wiping mode
      {
    ...
    rb_PotiStageFromBox_g = (l_u16_rd_bcm_rlsSensitivity() > 7)?7:(l_u16_rd_bcm_rlsSensitivity());
    
    if (l_bool_rd_bcm_wash_Status())                                    /* Washpump activ? 0x5:Front washing*/
    {
    ...
    getLightTransmission((l_u16_rd_bcm_500nmTransimission()),(l_u16_rd_bcm_800nmTransimission()));
  }  
  ...
  {                                                                     // check if system is on -> to switch off 
   if((rw_NoCommunicationCounter_g > CW_LIN_E2_IDLE_TIMOUT))                               /***4S***/
   {
      ...
      {
                                                       //auto close windows                         
        if ((!l_bool_rd_bcm_rlsAutomaticModeActive()) || (rw_CommandCloseWindowTimout_mg > 6000))
        {
...
void GetLsWWS(void)
{
  ...
  rb_Wischer_Parklage_l = l_bool_rd_bcm_frontWiperParkingSts();
...
void CopyRLSData(void)
{
  ...
  if(l_u8_rd_bcm_frontWiperStatus()==5)
  {
  ...
  if(l_flg_tst_RSM_state2())
  {
    l_flg_clr_RSM_state2();
    rbi_ConnectedToMaster_g = 1; 
  }
  if(l_flg_tst_RSM_state())
  {
    l_flg_clr_RSM_state();
    rbi_ConnectedToMaster_g = 1; 
  ...
  else  
  {
    if(l_u8_rd_bcm_frontWiperStatus()==5)
    {
      ...
      { 
         ...
         else if (wiperSpeed7to2(l_u8_rd_rsm_wiperSpeed(), rb_WiperSpeed_g, rb_wiperSpeedHysteresis) != 0)
         {
           newWiperSpeedLin = wiperSpeed7to2(l_u8_rd_rsm_wiperSpeed(), rb_WiperSpeed_g, rb_wiperSpeedHysteresis);  //continuous mode
         }   
      }
      else
      {
      ...
      l_u8_wr_rsm_wiperSpeed(newWiperSpeedLin);    
    }
    else
    {
      l_u8_wr_rsm_wiperSpeed(0);
    }   
...
void LightFunction(void)
{
 ...
 /* PRQA S 3415, 4112 1 */ /* << Getter functions must not have side effects >>*/
  if ((((((IS_LS_ANY_ERRORCOUNTER()) > 0))||((l_bool_rd_rsm_lightSensorError()) > 0))||(((rbi_UnderVoltage_g) == TRUE))
  ...
  if(l_bool_rd_rsm_lightSensorError() == TRUE)                                                                                 //sensor in error
  {
  ...
  { /* PRQA S 3415 1 */ /* << Swapping expressions not feasible for performance >>*/
    ...
    {
      rb_TempLightOn_l = l_bool_rd_rsm_lowBeamOnReq();                                                               /* stay in old light mode and do */
    }
  }
      ...
      && ((rb_RSControlFlags_g == cb_HUMIDITY_SOLAR )||(rb_RSControlFlags_g == cb_ONLY_HUMIDITY)))
  {
    ...
    l_bool_wr_rsm_lowBeamOnReq(TRUE);
    l_bool_wr_rsm_positionLightOnReq(TRUE);
  } 
  else
  {
    /* PRQA S 434, 436, 781 7 */ /* <<usind Micro Controller specific command > */
    DISABLE_SCIINTERRUPT();
    l_bool_wr_rsm_lowBeamOnReq(rb_TempLightOn_l);  
    l_bool_wr_rsm_positionLightOnReq(rb_TempLightOn_l); 
  ...
  l_u8_wr_rsm_lightSWReason(rb_ReasonForLightOn_l); 
}
#endif
...
void Humidity_Function(void)
{
  ...
  {
    l_u16_wr_rsm_dewPointTemp(0x7FF);                                //dew point temperature
    l_u8_wr_rsm_relHumidity(0XFF);      //Blake_Check 2018_03_10                             //relative Humidity
    l_u16_wr_rsm_windowTemp(0x7FF);                            //window temperature    
  }
  else
  {
    ...
    {
      l_u16_wr_rsm_windowTemp(0);
    }
    else
    {
      if(((rw_Temperature_Scheibe_g -725)/10) < 1395)
      {        
        l_u16_wr_rsm_windowTemp(((rw_Temperature_Scheibe_g -725)/10));
      }
      else
      {         
        l_u16_wr_rsm_windowTemp(1395);
      }
    } 
    ...
    else
    {
      if (rw_DewPoint_g < cw_DEWPOINT_MAX_mg )                /* dew point should be less than or equal to Scheibe temperature */
      {
        ...
        l_u16_wr_rsm_dewPointTemp(rw_DewPoint_g); 
        
      }
      else
      {
        l_u16_wr_rsm_dewPointTemp(cw_DEWPOINT_MAX_mg);
      }
    }
        
    if (rb_Humidity_g == cw_HUMTEMP_INIT_g)                   /* Relative Humidity output*/ /*VCO_061 name hase changed*/
    {
      l_u8_wr_rsm_relHumidity(0);
    }        
    else
    {        
      l_u8_wr_rsm_relHumidity(rb_Humidity_g);
    }
  }
}
#endif
...
void StandByMode(void)
{
  ...
  {
    l_bool_wr_rsm_windowCloseReq(1);                        // send close window command
  ...
  else
  {
    ...
    l_bool_wr_rsm_windowCloseReq(0);
  }
...
void OutPutBrightnessValue(void)
{
  ...
  if((rbi_LightValuesInitialised_g == FALSE)|| (rbi_EEPROMLoad_g == TRUE))
  {
    ...
    {      
      l_u16_wr_rsm_FwBrightness(0X7FE);                                  //FW INITIAL
      l_u16_wr_rsm_AmbBrightness(0x3FE);
      l_u8_wr_rsm_IRBrightness(0xFE);      
      l_u8_wr_rsm_SolarLeft(0xFE); /* Init/nicht verfuegbar */
      l_u8_wr_rsm_SolarRight(0xFE); /* Init/nicht verfuegbar */
    }
  }
  else
  {
    ...
    {
      /*Incase of only Feuchte and solar frame preset then set the light ON, FW and IR to init value for safety reason*/ 
      l_u16_wr_rsm_FwBrightness(0X7FF);                                  //FW INITIAL
      l_u16_wr_rsm_AmbBrightness(0x3FF);
      l_u8_wr_rsm_IRBrightness(0xFF);      
      l_u8_wr_rsm_SolarLeft(0xFF); /* Init/nicht verfuegbar */
      l_u8_wr_rsm_SolarRight(0xFF); /* Init/nicht verfuegbar */
    }
    else if(l_bool_rd_rsm_lightSensorError() == TRUE)                    /*  VDR469   set for error*/
    {
      l_u16_wr_rsm_AmbBrightness(0x3FF);      
      l_u16_wr_rsm_FwBrightness(0X7FF);
      l_u8_wr_rsm_IRBrightness(0xFF);
      l_u8_wr_rsm_SolarLeft(0xFF); /* Init/nicht verfuegbar */
      l_u8_wr_rsm_SolarRight(0xFF); /* Init/nicht verfuegbar */
    }
    ...
    {     
      ...
      {
        if(get_rw_FW_light16bit_g() < 0x7fd)                           /*VCO_031 correct variable 0x03FD*/
        {
           rw_FW_brightness_l = get_rw_FW_light16bit_g();
        }
        else
        {
          rw_FW_brightness_l = 0x7fd;
        ...
        l_u16_wr_rsm_FwBrightness(rw_FW_brightness_l);

        if(get_rw_Amb_light16bit_g()/2 < 1021)                           /*VCO_031 correct variable 0x03FD*/
        {
          l_u16_wr_rsm_AmbBrightness(get_rw_Amb_light16bit_g()/2);
        }
        else
        {
          l_u16_wr_rsm_AmbBrightness(1021);
        }        
      }
      
      if(rb_IrLightDiv8_g < 0xFD)
      {
        l_u8_wr_rsm_IRBrightness((byte)rb_IrLightDiv8_g);
      }
      else
      {
        l_u8_wr_rsm_IRBrightness(0xFD);     // IR Helligkeit max
      }
      ...
      if ((rbi_SoSe_Enabled_g == TRUE)&& (TRUE == rbi_SolarValuesInitialised_g))
      {
        ...
        l_u8_wr_rsm_SolarLeft(rw_SoSe_SunInt_2D_Le_l);
        ...
        l_u8_wr_rsm_SolarRight(rw_SoSe_SunInt_2D_Re_l);       
      }
      else
      {
        l_u8_wr_rsm_SolarLeft(0xfe); /* Init/nicht verfuegbar */
...
void VoltageDetect(void)
{
  ...
  {
    l_bool_wr_rsm_underVoltFault(1);
    l_bool_wr_rsm_overVoltFault(0);
  }
  else if(rbi_OverVoltage_g == TRUE)
  {
    l_bool_wr_rsm_overVoltFault(1);
    l_bool_wr_rsm_underVoltFault(0);
  }
  else
  {
    l_bool_wr_rsm_underVoltFault(0);
...
static void CheckErrors(void)
{
  ...
  l_bool_wr_rsm_rainSensorError((rb_RS_LS_Error1_g != 0)||(rb_RS_LS_Error2_g != 0)||(rb_RainS_Error1_g != 0) ||(rbi_HammingError_g != 0));  
  l_bool_wr_rsm_lightSensorError((rb_RS_LS_Error1_g != 0)||(rb_RS_LS_Error2_g != 0)||(rb_LightS_Error1_g != 0)||(rbi_HammingError_g != 0)||(rbi_CRC_ChangedByDiagnostic_g));
}
...
void ApplLinInit(byte rb_LIN_State_l)
{
  ...
  byte rb_Index_l; 
  static const byte LinMsgDefault_RLSs_01[8] = {0,0,254,254,254,135,255,254};   /* id: 0x011 */
  static const byte LinMsgDefault_RLSs_02[8] = {0,0,0,0,0,0,0,0};   /* id: 0x023 */
  
  rb_Index_l = 0;
  do
  {
    RSM_state._c[rb_Index_l] = LinMsgDefault_RLSs_01[rb_Index_l];
    RSM_state2._c[rb_Index_l] = LinMsgDefault_RLSs_02[rb_Index_l];
    rb_Index_l++;
  } while (rb_Index_l < 8);
...
__callt void ApplLinSlavePreCopy(byte rb_MessageID_l)                                     /* callt function faster */
{
  ...
  {
    l_bool_wr_rsm_ResponseError(rbi_latched_ResponseError_mg);
