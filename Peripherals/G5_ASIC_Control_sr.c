...
#else
#define TIME_FOR_RAINSENSOR_INIT   120
#endif
...
* \return void  
*/
byte Rain_Drop_Counter = 0,Three_Second_Time_Counter = 0,Time_Counter = 0;
static byte rb_OutdBri_Cnt_l = 1,rb_HUD_Cnt_s = 1,First_LIN_Wakeup_Flag = 0,LIN_Wakeup_for_Cntr = 0;
#ifdef G6_Standby
static void ASIC_Control_init(void)
{
  static byte Over_First_Prestandby = 0;
/*if the rain functionality is enabled*/
#ifndef _NO_RAIN_
...
/*if the rain functionality is enabled*/
  ...
  case CASE_ASIC_CNT_MD_NORMAL:  /*CASE_ASIC_CNT_MD_NORMAL -> Normal Operation*/
	Over_First_Prestandby = 1;
	Time_Counter = 0;
	Rain_Drop_Counter = 0;
	Three_Second_Time_Counter = 0;
	LIN_Wakeup_for_Cntr = 0;
	/*Not connected to Master? AND Old Cluster cleared?*/
	if ((rbi_ConnectedToMaster_g == FALSE)&&(rbi_GotoSleep_g == TRUE))
	{
...
/*if the rain functionality is enabled*/
	...
	if (rbi_ASICStandByModeRequest_g == TRUE)
	{
	  ...
	  if (rw_TimeOutCounter_s < (TIME_FOR_STANDBY_NORMAL_MEASURE - 350))
	  {
...
/*if the rain functionality is enabled*/
	...
	{
		//if(Enter_Prestandby_Add_Time == 1)
		//{
		//	rw_TimeOutCounter_s = TIME_FOR_STANDBY_NORMAL_MEASURE;
		//	Enter_Prestandby_Add_Time = 0;
		//}
	  ...
	  if(Over_First_Prestandby == 1)
	  {
		rw_TimeOutCounter_s = 0;
		Over_First_Prestandby = 0;
	  }
	  else
	  {
	  ...
	  if (rw_TimeOutCounter_s < (TIME_FOR_STANDBY_NORMAL_MEASURE - 280))
	  {
		...
		else
		{
		  ...
		  rw_TimeOutCounter_s = TIME_FOR_STANDBY_NORMAL_MEASURE; /* initialise Timoutdelay for first raincheck*/
#ifdef STANDBY_LIN_WAKE_UP_ISSUE
		  ApplLinBusSleep(); // VCO_477 added this for can no t be waked up when it is in standby mode at 2018.04.16
#endif
		}
	  }
	  else
...
/*if the rain functionality is enabled*/
	...
	if (rbi_ASICStandByModeRequest_g == TRUE)
	{
	  ...
	  /* Delay time for initialisation ready?*/
	  if (rw_TimeOutCounter_s < (TIME_FOR_STANDBY_NORMAL_MEASURE - 250))
	  {
		if (ASIC_Control_STBCheckRain()!=FALSE)
		{
		  rbi_Command_CloseWindows_g = TRUE;
    ...
    case 5:
      if((rb_ASIC_Sched_Config_g & 0x1) != 1)
      {
        if ((rb_ASIC_ALS2Amb_ShortDetected_cnt_g < rb_SC_order1_g) 
					    || ((rb_ASIC_ALS2Amb_ShortDetected_cnt_g >= rb_SC_order2_g) && (rb_ASIC_ALS2Amb_ShortDetected_cnt_g < rb_SC_order3_g)))
        {
    ...
    case 44:
      if((rb_ASIC_Sched_Config_g & 0x1) != 1)
      {
        if ((rb_ASIC_ALS5FW_ShortDetected_cnt_g < rb_SC_order1_g)
            || ((rb_ASIC_ALS5FW_ShortDetected_cnt_g >= rb_SC_order2_g) && (rb_ASIC_ALS5FW_ShortDetected_cnt_g < rb_SC_order3_g)))
        {
          /* Normal measurement order */
          /*change MUX value to ALS5*/
          rb_NextMUXCfg_g = CHG_MUX_ALS5;
    ...
    case 63:
      if (((rb_ASIC_Sched_Config_g & /*0xC0*/0xC1) == 0) || ((rb_ASIC_Sched_Config_g & 0x20) == 0x20))
      {
        if ((rb_ASIC_INRECIR_ShortDetected_cnt_g < rb_SC_order1_g)
            || ((rb_ASIC_INRECIR_ShortDetected_cnt_g >= rb_SC_order2_g) && (rb_ASIC_INRECIR_ShortDetected_cnt_g < rb_SC_order3_g)))
        {
          /*change MUX value to INREC*/
          rb_NextMUXCfg_g = CHG_MUX_REC;
          rb_NextSPI_Request_g = CB_CHG_MUX_STMEAS_RQ;
...
static void ASIC_Control_HUD_LightValScl(byte rb_filter_l)
{
  ...
  rl_V_ALS5_ASIC_Corr_l = ASIC_Control_LightValueCorrect(rw_ALS5_ASIC_g,rw_A_Factor_ALS5_g,CHG_MUX_ALS5);  /*VDR_131 2point calibration*/
